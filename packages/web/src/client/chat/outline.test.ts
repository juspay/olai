 ...
 *
 * The claim under test is the design's own "never a text diff of a `.org`",
...
const rewritten = (oldText: string | null, newText: string) =>
  outlineDiffOf({ path: "house.org", oldText, newText })
...
describe("an outline rewritten by hand", () => {
  test("a mark that appeared is the same word the Commit panel uses", () => {
    ...
    const answer = rewritten(orgFixture(`${OUTLINE}\n`), orgFixture(`${marked}\n`))
  ...
  test("a file that did not exist is every node created, not an unreadable side", () => {
    const answer = rewritten(null, orgFixture(`${OUTLINE}\n`))
    if (answer._tag !== "Changes") throw new Error("expected changes")
  ...
  test("a side that will not parse says which side, and offers no changes", () => {
    ...
    expect(rewritten(orgFixture(`${OUTLINE}\n`), `${orgFixture(`${OUTLINE}\n`)}not Org at all\n`))
      .toEqual({
      _tag: "Unreadable",
      side: "after",
    })
    expect(rewritten("not Org at all\n", orgFixture(`${OUTLINE}\n`))).toEqual({
      _tag: "Unreadable",
      side: "before",
    })
  })
  ...
  test("a rewrite that moved no record has nothing to report", () => {
    ...
    const physical = orgFixture(`${OUTLINE}\n`)
    const answer = rewritten(physical, `${physical}\n`)
