...
export const nodeMenuActions = (args: {
  ...
  /** Open the row's property editor — on an existing property, or on `null`
   *  for one being added. The same arrangement the two above are, and for the
   *  same reason: a key and a value are typed, and the panel belongs to the ROW
   *  rather than to a menu that is closed by the time either is. */
