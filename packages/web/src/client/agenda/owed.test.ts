...
/** One reading of a directory of two outlines — `work.org` is what the first
 *  argument holds, `life.org` the second, because a count of NODES has to be
 *  exercised across the groups an agenda comes in. */
const readingOf = (work: ReadonlyArray<string>, life: ReadonlyArray<string> = []) =>
  agendaOf(
    derive(
      nodesOfFiles({ "work.org": work.join("\n"), "life.org": life.join("\n") }),
    ),
    TODAY,
  )
