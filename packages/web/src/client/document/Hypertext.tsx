 ...
 * as a CLAIM: {@link HEIGHT} is a CSS `clamp` around it, so a frame is never
 ...
 * measured page is allowed to land in, and what stands in when no page has
 * measured itself.
 *
 ...
 *
 * `unmeasured`: what the frame is before the first measurement arrives, and
 * what it stays as if none ever does — an old browser, a policy that refused
 * the measure, a page that hung before `DOMContentLoaded`. It is the number
 * this component used unconditionally before, so the failure mode of the new
 * mechanism is exactly the behaviour of the old one. Not a bound: the clamp
 * applies to it like any other value, and it sits between the two.
 *
...
export function Hypertext(props: { readonly file: string; readonly text: string }) {
  ...
  // panel widths use): that one takes three numbers, and two of these three are
  // `dvh` — a length this component does not have and should not be reading. A
  // clamp the browser evaluates is also one that follows a rotated phone
  // without anybody being told.
  //
  // Setting the height from a measurement made INSIDE the box being sized is a
  // loop, and this is where it closes; it terminates because of which reading
  // the measure takes. `documentElement.offsetHeight` is the content's height
  // and does not know the frame's, so growing the frame does not grow the
  // number (`./sealed.ts` argues the choice). A page reflowing to a new WIDTH
  // does report again, which is the point — that one converges too, because a
  // frame sized to its content has no scrollbar left to take width away.
  const height = () =>
    `clamp(${HEIGHT.min}, ${measured() === undefined ? HEIGHT.unmeasured : `${measured()}px`}, ${HEIGHT.max})`
