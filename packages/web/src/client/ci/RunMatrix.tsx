...
const spanOf = (cell: RunCell, now: number): string => {
  ...
  if (cell.status === "running" && cell.startedAt !== null) {
    return tickingOf(now - cell.startedAt)
  }
  return "—"
}

/** ...and the exact figure on the hover, where there is one to be exact about. */
const spanTitle = (cell: RunCell): string | undefined =>
  cell.ms === null ? undefined : `took ${exactOf(cell.ms / 1000)}`
...
function Matrix(props: { readonly run: CiRun }) {
  ...
  const now = createTicking(SECOND)
  const head = () => {
    const run = props.run
