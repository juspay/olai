 ...
 *
 * `@olai/kolu-ui`'s `fleet.tsx` one appliance over, and the economy is the
 * same: a lanes outline has a `worktree` property on a dozen rows, and every
 ...
 * subscribe, and this context is what it reads instead. It is `../served.tsx`'s
 ...
 * runs are bounded by the lanes a board is running at once, which is a
 * handful, and the cell's `equals` (`@olai/surface`'s `sameCi`) already
...
import { type CiRun, type CiRuns, NO_RUNS } from "@olai/surface"
...
export function RunsProvider(props: {
  ...
  /** ONE memo per tab over the whole reading, so a page of twelve lanes does
