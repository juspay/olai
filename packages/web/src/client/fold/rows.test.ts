...
const derived = derive(
  recordsOf(setOf({ "house.org": HOUSE, "garden.org": GARDEN })),
)

const house = rowsOf(derived, "house.org")
...
test("a node folds by itself, in the file it is written in", () => {
  expect(foldOf(kitchen)).toEqual({ id: "kitchen", file: "house.org" })
...
test("a mirror folds by what it SHOWS, in the file that node lives in", () => {
  ...
  // house.org and the fold is `herbs` in garden.org, which is what makes
