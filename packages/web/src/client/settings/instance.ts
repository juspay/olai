/**
 * WHAT A ROW SAYS WHEN IT IS THE INSTANCE'S AND FROZEN — the two sentences a
 * read-only preference row shares, and the one place they are spelled.
 *
 * Most rows on this panel are a claim about the READER and are stored in this
 * browser. The two git rows are not: they are set where the server is started —
 * a flag, or the nix module that passes the same flags — so a browser DRAWS
 ...
 * would have made a second caller import "the git policy" to say a sentence
 * that is not about git.
 *
 * ## THE PLUGIN ROWS WERE THAT SECOND CALLER, and are not any more
 *
 * They read exactly like the git rows until the plugins panel got a switch (the
 * human, 2026-09-04): a source line, a frozen strip, the same two sentences.
 * What ended it is the second half of each — *it cannot be changed from a
 * browser* — which a live switch makes false. The choice was to soften the
 * shared clause or to stop sharing it, and softening it would have softened it
 * for the two rows that still mean it exactly.
 *
 * So `../plugins/rows.ts` spells its own, and this module is the git rows'
 * alone again. The argument above is unchanged and is why it stays a module: the
 * day a third genuinely-frozen row arrives it takes these sentences verbatim.
 *
 ...
 * type it, `--commit=auto`, `--push=on`.
 *
 * It takes the whole spelling rather than a name and a value, because not every
 * flag's value is a word: a flag given with nothing after it is a legal value
