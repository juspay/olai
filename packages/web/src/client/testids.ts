 ...
 *
 * Three groups left this table for that reason and their absence is worth a
 * line each. The `padi*` five went to `olai-plugin-kolu` with the pill and its
 * drawer, and the `ci*` three to `olai-plugin-odu` with the chip and its
 * matrix. The third is by far the largest and is the phase this paragraph was
 * last edited for: 151 ids went to `olai-plugin-chat` when the chat panel
 * became a plugin — the whole `chat*` run, the sidebar's `agent*` roster and
 * the `unassigned*` / `assign*` migration list, every one of them drawn by a
 * file under that package's `src/browser/`. Their VALUES did not change, which
 * is what kept the move from also being a rename: a testid is a promise to a
 * scenario, and a relocation that renamed at the same time would have made
 * every red line in the suite ambiguous between two edits.
 *
 * EXACTLY ONE `chat*` NAME STAYED, and it is the clearest statement of what the
 * rule actually is: {@link panelResize} is the drag handle on the dock's left
 * edge, drawn by this client's own `layout/Handle.tsx`, so the word in the id
 * says "chat" and the renderer says `@olai/web` — and the renderer is what
 * decides.
 *
 * A fourth group was DELETED rather than moved: six `terminal*` ids that
 * duplicated that tenant's own, value for value, left behind when the door
 * became a package — nothing in this client or the suite ever read them, and
 * two spellings of one contract is the exact failure this file exists to
 * prevent.
 */
...
export const TESTID = {
   ...
   *  (`client/layout/Handle.tsx`).
   *
   *  THE ONE `chat*` NAME LEFT IN THIS TABLE, and it is here for the rule
   *  rather than in spite of it: what moves is decided by which package
   *  RENDERS the element, not by the word in the id. This handle is chrome
   *  about the COLUMN — how wide the app makes it, remembered per browser —
   *  and the conversation drawn inside it is `olai-plugin-chat`'s whole face.
   *  The plugin never draws this, so it never owned it. */
...
/**
 * AN ID FROM EITHER TABLE, named — because a SHARED WIDGET has to be able to
 * take one.
 *
 * `selector` above spelled the union inline while it was the only thing that
 * needed it. It stopped being the only thing when the chat panel became a
 * plugin: `search/Shortlist.tsx` is one search box four doors dress with their
 * own names, and one of those doors — the *assign to node…* list — is
 * `olai-plugin-chat`'s now. A `ShortlistTestids` typed `TestId` would have
 * meant *only core may dress the shared widget*, which is not a claim anybody
 * made; it was the accident of the union having no name.
 *
 * It is a UNION and not `string`, which is the whole of the guard and the
 * shorter edit that was refused: both halves are closed sets, so a typo is a
 * type error at the call site rather than a selector that quietly matches
 * nothing for thirty seconds. Widening to `string` would have bought the same
 * three call sites and thrown that away.
 */
export type AnyTestId = TestId | PluginTestId
