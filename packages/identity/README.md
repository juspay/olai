...
This package is the PERSON as a value. It is not a cell (a cell is one value for the process), it is not HTTP, and it is not a surface member. `identityOf` is the one reading; `headerNamesOf` is the allowlist `serveSurfaceApp` takes (unique, because login and email often share a name). `GET /olai/who` is `@olai/server`'s HTTP door over that reading; `who.get` is the tab's door, answered from the upgrade. The path and JSON live in `@olai/surface`, the way `/media` does. The retired `POST /capture` called the same function on its own request rather than growing a second parse; `/mcp` reads the same headers now, per REQUEST, which is what makes a capture from a terminal or an agent behind a proxy attributable at all — and attributable to nobody, honestly, when there is no proxy in front to name anybody.
