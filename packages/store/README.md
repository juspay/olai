...
**A refusal at step 4 is not automatically THIS write's.** The codec judges the whole set, because that is what a set means; whether the judgement is ABOUT the files this commit puts down is a second question, and it is the codec's too (`Codec.admits`, optional — absent means "no", which is every codec's behaviour before it existed). A codec that answers it lets a write to healthy files land beside a file that will not validate, with the last good snapshot still standing and the refusal still on the errors channel — the same per-file degradation a READ has had since 2026-08-09, which writes never got. This package looks inside neither the `E` nor the files: it hands over what it has and spends the boolean. What it does NOT ask the codec is whether the write is safe to make from the base the caller planned against — a path the published revision no longer accounts for cannot be written from it, which is this package's own bookkeeping and is checked first, because while a set will not validate the snapshot does not move and a second write would otherwise be planned off a copy the first one never reached.
