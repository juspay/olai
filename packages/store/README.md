...
5. **re-probe and publish**. The changed files are re-read because the gate says so, not because a stat noticed: stamps are mtime+size, and a write that lands in the same second at the same length is exactly what they cannot see. For a change from outside that is the accepted trade; for one this process just made it would mean a browser never getting the frame.
