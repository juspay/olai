...
# that the engines are plugins. The shared mechanism is `nix/acp-agent.nix`
# (one derivation per adapter from the shared `acp/` shim); THIS file owns the
# Claude row's facts: the npm package inside the shim tree, the wrapper's bin,
# the env it arms (pinned `claude` executable, no autoupdater or install checks,
# ripgrep from the store), and the Linux patchelf of the bun-compiled `claude`
# binary the SDK ships.
#
...
# aliased to `claude-agent`, the package this file exports.
...
let
  ...
  adapter = acpAgent {
    ...
    patches = ./acp/patches;
    # patchelf for the SDK's bun-compiled `claude`; only the interpreter may be
    # touched (see nix/acp-agent.nix's header).
    nativeBuildInputs = lib.optional pkgs.stdenv.hostPlatform.isLinux pkgs.patchelf;
