...
Turning this row off without turning chat off is `--plugins`: `olai web --plugins=vault,chat,opencode,pi` serves a panel with no Claude row, no probe for one, and no mark for one anywhere. It is one row of `olai.yml` like any other plugin ([the plugins panel](../running.md) draws it with the same five states).
