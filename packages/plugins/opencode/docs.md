...
Turning this row off is `--plugins`: `olai web --plugins=vault,chat,claude` serves a panel with no opencode row and no probe for one. It is one row of `olai.yml` like any other plugin.
