/**
 * WHAT IS CONTRIBUTED AT A LOCATION, as the router and the palette reads it — held for the
 * activation that declared the renderer.
 *
 * The reading is `olai-plugin-ui-renderer`'s `RendererSlots.read`, and it
 * arrives on a service this package already names on its `renderer` component. It used to arrive as
