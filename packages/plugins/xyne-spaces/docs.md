...
**Off by default.** Omitting `--plugins` runs the default bundle; this plugin stays off until a flag names it. `--extra-plugins=xyne-spaces` is the flag that does that without listing everything else; `--plugins=vault,chat,claude,xyne-spaces,ws,web-app,mcp,ui-renderer,layout` is the exact set, and turns off every row it does not name. A Spaces app JWT is a secret this machine may not have, and a pill in every bar for an integration nobody pointed at is the wrong default.
