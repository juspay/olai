...
export const useServed = (): Accessor<ReadonlyArray<string>> => {
  const files = useContext(ServedContext) ?? held?.paths
  if (files === undefined) {
...
export const useHead = (
  ...
  const head = useContext(HeadContext) ?? held?.head
  if (head === undefined) throw new Error("a head lookup outside <ServedProvider>")
  return head(file)
}

let held: {paths: Accessor<ReadonlyArray<string>>; head:(file:Accessor<string>)=>Accessor<number|undefined>} | undefined
export function holdServed(value: NonNullable<typeof held>): () => void {
 held=value; return ()=>{ if(held===value) held=undefined }
}
