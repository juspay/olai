 ...
 * still offered an agent an `empty_trash` nothing stood behind. A tool leaves
 * with the row that owns it now (juspay/olai#546).
 *
 * THE UNIT IS THE WHOLE PILE, and that is the rule rather than an exception to
 * it: `empty_trash` names the `_olai/Trash.olai` the set already serves and
 * empties it whole — every record in it or none, and nothing that could name
 * part of one. `untrash_node` is the other half, and the only undo a RECORD
