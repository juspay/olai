...
import { Edges0, Empty } from "./absent.tsx"
...
export function GraphFace(props: {
  ...
  /** The one ruling on whether there is a picture to show at all — the
   *  arm attribute and the branch below read the SAME answer, never two
   *  spellings of it. */
  const drawable = () => refused() === undefined && held() <= GRAPH_DRAWN_AT_MOST
  ...
  const arm = () => (drawable() ? "picture" : "sentence")
...
function Shape(props: {
   ...
   *  already the face's memo, so this is an ALIAS rather than a wrap. */
