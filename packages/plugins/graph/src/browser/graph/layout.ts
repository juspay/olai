 ...
 * the settled positions are mapped into the {@link Frame} PER AXIS — which
 * would stretch the shape if it were the last word, and is not: the circular
 * collision pass after it re-rounds every neighbourhood ({@link fitted} has
 * the argument, and it is one paragraph because the per-axis stretch was the
 * one place a uniform scale was wrong, not the first).
 *
...
import { byPath, type Edge, type Vertex } from "@olai/format"
import {
  forceCollide,
  forceLink,
  forceManyBody,
...
/**
 * THE TWO LISTS IT READS — the vertices and edges off the page's narrowed
 * drawing (`olai-plugin-outlines/page`'s `Drawn`), which is all a layout may
 * ask: not the filter that pruned it, not the centre, not `held`. What a
 * keystroke pruned is simply not laid out, and a picture that DOES fit after
 * a pruning settles again because {@link sameShape} says so, not because a
 * record changed.
 */
...
export const placed = (graph: Shaped, frame: Frame): Placement => {
  if (graph.vertices.length === 0) return NOTHING_PLACED

  const files = [...new Set(graph.vertices.map((vertex) => vertex.file))].sort(byPath)
  // A file's NAME is written only where something a person would name an
  // outline by lives in it: a node, or the outline itself as a vertex. A
  // lone document vertex draws no label — its dot already carries the name —
  // but it still PULLS toward its file, which is why `anchors` is over every
  // file and this set is only for the label.
  const named = new Set(
    graph.vertices
      .filter((vertex) => vertex.kind === "node" || vertex.kind === "outline")
  ...
  const bodies: Array<Body> = graph.vertices.map((vertex, index) => {
    const toward = anchors.get(vertex.file)!
  ...
  return fitted(bodies, frame, named)
}
 ...
 */
export const sameShape = (one: Shaped, other: Shaped): boolean => {
  if (one === other) return true
  if (one.vertices.length !== other.vertices.length) return false
  if (one.edges.length !== other.edges.length) return false
  for (const [index, vertex] of one.vertices.entries()) {
    const mine = other.vertices[index]!
    if (mine.key !== vertex.key || mine.file !== vertex.file || mine.kind !== vertex.kind) return false
  }
