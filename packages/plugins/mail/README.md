...
olai's own judgement **about a Gmail mailbox**, in the one place that is neither Gmail nor core. Phase 1 is Connect: a serve holds one account, an OAuth refresh token lives in core's memory door, and the header pill says which mailbox it is reading.
...
Tools, the `mail-thread` kind and the doorbell are the next three steps of the same plan; sending, drafts and permanent delete are ruled out (`docs.md` says why). A second account is a migration this phase does not need, because the property values that will name a thread carry the account from the first day.
