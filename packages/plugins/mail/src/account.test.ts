...
import { type AccountMachine, DOORS, LEAD_MS, makeAccount, PENDING_MS, profileOf, wakeIn } from "./account.ts"
...
const harness = (over: {
  ...
  let held: Record<string, unknown> = over.record === undefined ? {} : { ...over.record }
  const door: LocalState = {
    load: Effect.suspend(() => Effect.succeed(held.refreshToken === undefined ? null : held)),
    save: (value) => Effect.sync(() => { held = value }),
  }
  ...
  const machine = makeAccount({
    ...
    endpoints: GOOGLE,
    himalaya,
    memory: Effect.runSync(openMemory(door, (line) => warnings.push(line))),
    post: async (request) => {
  ...
  return {
    ...
    posts,
    warnings,
    get memory() { return held.refreshToken === undefined ? undefined : (held as unknown as MemoryRecord) },
    closed: () => closed,
    at: (next) => { iso = next },
...
test("a serve with no record is absent, and says whether Connect could work", async () => {
  ...
  expect(last(short.painted).reason).toContain("OLAI_MAIL_OAUTH_CLIENT")
  expect(last(short.painted).reason).toContain("OLAI_MAIL_OAUTH_SECRET")
})
...
test("a profile this plugin cannot read is a fault rather than a connected lie", async () => {
  const ready = harness({ record: RECORD, profile: { "messages-total": 12 } })
  await Effect.runPromise(ready.machine.boot())
  expect(last(ready.painted).status).toBe("fault")
  expect(last(ready.painted).reason).toContain("profile")
})

test("a Himalaya that refuses the profile is a fault carrying its sentence", async () => {
...
test("begin refuses without the credentials, naming both doors", async () => {
  ...
  if (Result.isFailure(outcome)) {
    expect(outcome.failure.reason).toContain(DOORS)
  }
...
test("the refresh wakes five minutes before the token expires, never in the past", () => {
  const now = Date.parse(CONNECTED_AT)
  expect(wakeIn({ expiresAt: now + 3600_000, now })).toBe(3600_000 - LEAD_MS)
  expect(wakeIn({ expiresAt: now + LEAD_MS, now })).toBe(0)
  expect(wakeIn({ expiresAt: now, now })).toBe(0)
})
