...
test("a selected browser-only row does not claim successful browser activation", () => {
  expect(pluginHint({ name: "shell", running: true, browserOnly: true })).toBe("Selected by the host; runs only in the browser.")
  ...
  expect(browserHint("shell", new Map([["shell", { state: "running" }]]), true)).toBe("Browser: running.")
