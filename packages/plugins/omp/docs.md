...
The ACP engine olai **finds**. Put [`omp`](https://github.com/can1357/oh-my-pi) on this server's agent search path and the panel offers it. When it is missing, its enabled plugins row moves to **Needs you** with installation advice and a link. Whenever an engine menu opens, omp appears disabled with the same reason but no link inside the disabled choice. After installing it, switch the plugin off and on to probe again without restarting olai. Olai ships no pin or adapter override for it; `OLAI_AGENT_PATH` (defaulting to the server's `PATH`) selects the build.
...
- **the tool's name is the head of the call id** (`write:0`, `bash:3`), which is also the key a permission request arrives under. The `title` is the model's own intent sentence for a `write`, or the `$ <command>` line for a `bash` — description, never a name — so it is never what a row is named by.
- **its shell commands run as olai's own processes.** Because the panel advertises `terminal: true`, omp asks olai to run a command (`terminal/create`), names the handle on the call's `{ type: "terminal" }` block, and the output you watch stream under the row is a process spawned in the served directory — so it is olai, not omp, that stops it when you cancel or the session ends ([chat.md](../chat.md) has the same contract in the client's words). There is no `terminal_output` `_meta` here, which is why this engine does not advertise that extension and why the row streams anyway.
