...
Turning this row off is also possible through the plugin roster: `olai web --plugins=vault,chat,claude,opencode,pi,ws,web-app,mcp,ui-renderer,navigation,layout,outlines,markdown,files,sidebar,preferences,theme,plugin-inspector` mounts no Codex server half, performs no probe and loads no Codex browser chunk.
...
- **Subagents have their own cards and work doors.** Native child-session announcements identify each agent and its parent, including nested agents. Child tool calls appear inside that agent's work; its prose stays in the agent card's fold. Permissions and questions remain visible in the main conversation and name the child asking. Child settings, titles and usage never replace the main session's.
