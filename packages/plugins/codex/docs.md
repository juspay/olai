...
Turning this row off is also possible through the plugin roster: `olai web --plugins=vault,chat,claude,opencode,pi,ws,web-app,mcp,ui-renderer,layout` mounts no Codex server half, performs no probe and loads no Codex browser chunk.
