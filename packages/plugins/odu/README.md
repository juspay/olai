...
olai's own judgement **about odu**, in the one place that is neither odu nor core. [`olai-plugin-odu/appliance`](src/appliance/README.md) is how olai *reaches* odu — the service link, the board and stream holds, the projection into olai's shapes. This package is everything downstream of that which still says the word: what the reading is called on the wire, what a person is shown when a lane names a run, which runs a scoped conversation is woken for, and whether odu's own MCP is here. [`@olai/plugin-api`](../../plugin-api/README.md) is the door both tenants come through; this is the smaller of the two behind it.

## The wall below this one does not move

`@odu/*` is imported in exactly one package and it is not this one. The appliance dials odu's per-user service, holds the board and one nodes stream per live boarded run, and folds odu's own `STATUS_META` where that table actually lives — so a change to odu's contract is a change **there** and stops. `packages/bundle/src/fence.test.ts` asserts it with zero exceptions, deriving the tenant from the registry rather than carrying a list — which is what admits a plugin package that legitimately names its own appliance. What crosses into here is olai's vocabulary — a `CiRun`, a `RunCell` — and nothing of odu's.
...
Odu contributes ONE property kind ([`src/kinds.ts`](src/kinds.ts)): `run`. A vault declares it in `_olai/Properties.olai` like any other type, and that row is what licences the chip. The value is an odu run id (`<base36>-<base36>`).

The vault WALK that reads it is [`src/boarded.ts`](src/boarded.ts). It belongs here because what it decides is which run ids olai asks the service about. It may not live in the appliance — it reads outline records, and that package must not learn what one is — and it has no business in core, which holds the vault but makes no judgement about odu. What crosses is the id strings, handed to `revision`. Between those two is what this package is.
