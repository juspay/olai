...
- **`resolve.ts`** — WHERE a `worktree` value actually is. A relative value does not name its repository, so the rule is: an absolute value as written; a relative one joined onto `<repos root>/<repo>`, where the repo is parsed out of the node's own `pr-url`; where there is no PR URL, a repository the vault walk handed over (from the row's `projects/<repo>/` file) is spent the same way; and the repos root is the served directory's parent unless `$OLAI_REPOS_DIR` says otherwise. A value the rule cannot place resolves to nothing and is not probed. Nothing here touches the filesystem — it says where to look, and the dial finds out.
- **`runs.ts`** — the SWEEP and the hold. Nothing tells olai that a run has started (`.ci/odu.sock` appears in a worktree the server may not even be serving), so the only way to learn is to ask: every three seconds, one `connect(2)` per un-held worktree. A dial that lands is held open by its own fiber until the socket dies, following two cells at once — the pipeline and the header, which odu publishes twice per run — and ringing `rang` on the two transitions a conversation is owed: first-red once per hold, and settle when THE RUN settles — the frame whose every node is terminal, the same reading odu's own `wait_for_settle` takes off the same cell. Reading the socket's death for it instead was the watch's founding approximation, exact before odu grew `--linger`: a lingering coordinator holds the socket open PAST the settle on purpose, and the settle must be said when the run says it. The socket's death keeps one reading: a run that never settled on the hold DIED.
