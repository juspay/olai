/**
 * ODU'S BROWSER HALF — a Cordis plugin, exactly the shape its server half is.
 *
 * ## What this replaced
 *
 ...
 * ticking is a test suite. `wired` is this plugin's own sibling client, keyed
 * by the fiber so it cannot be asked for under another plugin's name.
 *
 ...
 * honest narrowness the `OduApp` re-declaration had, said to the runtime
 * instead of to the type checker.
 */
...
export function apply(ctx: Context): void {
  ...
  //
  // THE WORD IS THIS PLUGIN'S KIND and is passed BARE: `ctx.slots` composes it
  // with the fiber's name, exactly as `ctx.kinds` composes it on the server, so
  // the word a face is looked up by and the word a vault declares cannot be two
  // spellings. It is the same constant the probe walk follows and the value
  // gate holds a declaration to — one spelling, one authority, and the chip and
  // the dial cannot come apart.
  ...
  ctx.slots.register("app.mount", (props) => (
    ...
    // guard because there is nothing to check: the value came from the
    // framework's own client bundle under this plugin's key.
    <ClocksProvider clocks={ctx.clocks}>
      <RunsProvider runs={(ctx.wired.client() as CiClient).cells.ci.use().value}>
        {props.children}
      </RunsProvider>
    </ClocksProvider>
  ))
