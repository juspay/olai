...
import { attachmentRejection, VIDEO_TYPES } from "@olai/surface"
 ...
 * agent reads the file's kind from. A pasted recording is named from the
 * gate's own video table: calling an unnamed `video/mp4` `pasted.png` would
 * get it past the gate as a picture it is not.
 */
const nameOf = (file: File): string => {
  if (file.name !== "" && file.name.includes(".")) return file.name
  const video = VIDEO_TYPES.find((video) => video.types.includes(file.type))
  if (video !== undefined) return `pasted${video.extension}`
