...
Then("the armed node {string} is labelled {string}", async function(this: OlaiWorld, id: string, title: string) {
  await this.waitUntil(async () => (await armed(this, id).innerText()).includes(title), `armed ${id} to show ${title}`);
});
