...
Feature: Node agent idle timers preserve read conversations and durable conversations
  ...
  Rule: Boot has no foreground conversation
    Scenario: Stored conversations do not wake a node agent before a tab opens one
      Given I open the outline "lanes.olai"
      Then all bound node agents are asleep
      And there should be no page errors

  @scratch:chat
  Rule: Reading and ongoing work own the live scopes
    Background:
