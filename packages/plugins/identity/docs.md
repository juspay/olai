...
**One thing does need a restart**, and it is the websocket. The headers a socket may carry are fixed when the port binds, so a serve that came up *without* this row and then switched it on at the panel answers `GET /olai/who` and attributes `/mcp` writes immediately, while the chip in an open tab stays anonymous until the process restarts. Switching the row off is immediate everywhere; switching it back on within one serve is too. It is the same bargain the engines take with the chat roster, and it is the one seam of the whole move.
