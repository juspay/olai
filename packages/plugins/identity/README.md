...
**One seam, and it is the socket's.** The headers a websocket may carry are fixed when the port binds, so a serve that came up without this row and switched it on at the panel answers the two HTTP doors immediately while an open tab stays anonymous until the process restarts. Switching it off is immediate everywhere. `@olai/plugin-api`'s `Identity` argues the shape; `docs.md` says what it costs.
