/** The search browser capability's scoped client. Importing the contract
...
export function holdClient(read: () => Client): () => void {
  if (held !== undefined) throw new Error("search client already acquired")
  held = read
  return () => { if (held === read) held = undefined }
}
export function client(): Client {
  if (held === undefined) throw new Error("search client read outside its activation")
  return held()
}
