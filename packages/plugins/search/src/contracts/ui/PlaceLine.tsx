...
export function PlaceLine(props: { readonly place: Place; readonly testid?: string }) {
  return <span class="flex w-full min-w-0 overflow-hidden font-mono text-[0.6875rem] text-muted" data-testid={props.testid}>
    ...
    <Show when={props.place.middle}>{middle => <span class="min-w-0 truncate" data-place="middle"><Separator /><TitleHtml drawing={renderTitle(middle(), "", { links: false })} /></span>}</Show>
    <Show when={props.place.nearest}>{nearest => <span class="max-w-[45%] shrink-0 truncate" data-place="nearest"><Separator /><TitleHtml drawing={renderTitle(nearest(), "", { links: false })} /></span>}</Show>
  </span>
}
