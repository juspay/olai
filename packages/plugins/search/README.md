# olai-plugin-search — the matcher, as a row

Search is a plugin ([docs/plugins/search.md](docs.md)): one row in the build, on by default. Core defines a `Search` door ([`@olai/ops`](../../ops/README.md)) and this row stands behind it, so a serve that does not name the row keeps the grammar, the tool and every box a person types into — and answers all of them with no hits and the reason.

What this package holds is the two halves of that answer. This README is the TABLE's, because that is the half with libraries surveyed and an algorithm to explain; `docs.md` is the row's, and [docs/search.md](../../../docs/search.md) is what a query MEANS.

Every door that searches the DIRECTORY — an agent's `search_nodes`, the `⌘K` palette, the header's box, the chat composer's `@` list — used to read every record and every body in the vault, once per query. `src/table.ts` is what they read instead: a trigram table over the same folded text, kept level with the store's revisions, answering with the records and documents a query *might* select. `src/matcher.ts` is the walk over it — `@olai/ops`' `Query.search` until this row existed — and `src/server.ts` is the twenty lines that open one table on the plugin's own scope and offer the door.
...
The design, with the eleven libraries that were surveyed and why none of them can be used, is [brainstorming/search-index.md](https://github.com/juspay/oss.olai/blob/main/projects/olai/brainstorming/search-index.md).

## The whole surface

```ts
import { open } from "./src/table.ts"
...
`just bench` (over [`src/table.bench.ts`](src/table.bench.ts)) prints the band. On a generated vault of 1,000 outlines — 21,552 records — and 200 documents holding 0.77 MB of prose:
...
The design priced this engine **on disk** — "index ≈ 3× the text", with *zero process memory* as the reason to put it there ([brainstorming/search-index.md](https://github.com/juspay/oss.olai/blob/main/projects/olai/brainstorming/search-index.md)). This implementation puts the same postings **in memory**, held for the life of the process. That trade is deliberate — a file beside somebody's vault is a file to invalidate, version, garbage-collect and explain — and it went unpriced until a reviewer said so, so the bench prints it:
...
- **No `Vault` in `needs`.** The reading comes IN, through the door, per ask — with the clock the question is asked at and the kind vocabulary this serve runs. A row that read the vault for itself would answer a revision of its own choosing, which is the one window the third rule above exists to close, arriving from the plugin side.
- **No wire.** This row composes no sibling surface: `search.nodes` is core's member on core's three faces, and the `search_nodes` tool lands through it. What this row contributes is the answer behind them.
