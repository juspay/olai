/** A document's metadata is keyed by its file. A fragment remains on the
 * navigation route for landing and history, never on the metadata request. */
import { type Claims, bodyKind, DocumentPath } from "@olai/format"
import type { DocumentPageRequest } from "@olai/format"
import type { Route } from "olai-plugin-navigation/contract"

export const documentFile = (claims: Claims, route: Route): DocumentPath | undefined => {
  if (route.kind !== "at" || route.address === null || route.address.kind === "node") return undefined
  const path = route.address.path
  return bodyKind(claims, path) === null ? undefined : DocumentPath.make(path)
}

export const documentRequest = (claims: Claims, route: Route): DocumentPageRequest | null => {
  const path = documentFile(claims, route)
  return path === undefined ? null : { kind: "at", address: { kind: "document", path } }
}
