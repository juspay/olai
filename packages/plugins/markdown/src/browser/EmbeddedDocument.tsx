...
export function EmbeddedDocument(props: {readonly file: string}) {
  ...
  return <>
    <Show when={served()}>{entry => <Markdown revision={servedDirectory()?.paths()} serves={path => servedDirectory()?.members().has(path) ?? true} claims={servedDirectory()?.claims()} source={proseIn(entry().text)} from={props.file} class="olai-md-compact" testid={TESTID.documentBody} />}</Show>
