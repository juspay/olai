 ...
 * still advertised `read_document` to an agent nothing would answer. A tool
 ...
 * serves and carry its text ENTIRELY — `write_document` through the same plan →
 * validate → stage → rename → commit gate every other write goes through,
 * `read_document` out of the same snapshot every other read answers from — so
 * there is no offset, no range and nothing for a caller to splice at either
 * end. `list_documents` is that closure said out loud: what an agent may read
 ...
 * an agent could `create_document` a file it could never read back, and
 * `write_document`'s `was` — the conditional write that refuses to land on
