...
test("a bare name takes the door's own suffix", () => {
  expect(meantAt(TEST_CLAIMS, "olai", "Foo")).toEqual({ file: "Foo.olai" })
...
test("a path with folders in it keeps them and takes the suffix", () => {
  expect(meantAt(TEST_CLAIMS, "olai", "notes/plan")).toEqual({ file: "notes/plan.olai" })
...
test("a name already carrying the door's suffix is taken exactly as it is", () => {
  expect(meantAt(TEST_CLAIMS, "olai", "Foo.olai")).toEqual({ file: "Foo.olai" })
  expect(meantAt(TEST_CLAIMS, "olai", "plans/next.olai")).toEqual({ file: "plans/next.olai" })
...
test("an unclaimed suffix reaches the server unchanged for its refusal", () => {
  expect(meantAt(TEST_CLAIMS, "olai", "plan v1.2")).toEqual({ file: "plan v1.2" })
  expect(meantAt(TEST_CLAIMS, "olai", "Foo.txt")).toEqual({ file: "Foo.txt" })
  expect(meantAt(TEST_CLAIMS, "markdown", "archive.tar.gz")).toEqual({ file: "archive.tar.gz" })
})
...
test("a name carrying another kind's suffix is refused in the box's own words", () => {
  expect(meantAt(TEST_CLAIMS, "olai", "notes.md")).toEqual({
...
test("the third kind is named too, in the word a reader is given for it", () => {
  expect(meantAt(TEST_CLAIMS, "olai", "report.html")).toEqual({
...
test("a name that is only a suffix is refused without advice to type nothing", () => {
  expect(meantAt(TEST_CLAIMS, "olai", DOCUMENT_EXT)).toEqual({
...
test("a path the ops layer will refuse is completed and passed on all the same", () => {
  expect(meantAt(TEST_CLAIMS, "olai", "../escape")).toEqual({ file: "../escape.olai" })
  expect(meantAt(TEST_CLAIMS, "olai", "/etc/passwd")).toEqual({ file: "/etc/passwd.olai" })
})
...
test("a last segment that names a place, not a file, is not completed at all", () => {
  expect(meantAt(TEST_CLAIMS, "olai", "..")).toEqual({ file: ".." })
  expect(meantAt(TEST_CLAIMS, "olai", ".")).toEqual({ file: "." })
  expect(meantAt(TEST_CLAIMS, "olai", "foo/..")).toEqual({ file: "foo/.." })
  ...
  expect(meantAt(TEST_CLAIMS, "olai", "notes/")).toEqual({ file: "notes/" })
...
test("a name that begins with a dot is a file, and takes the suffix", () => {
  expect(meantAt(TEST_CLAIMS, "olai", ".plan")).toEqual({ file: ".plan.olai" })
...
test("an empty box, or one holding only spaces, has asked for nothing", () => {
  expect(meantAt(TEST_CLAIMS, "olai", "")).toBeNull()
