...
The file is normal outline records. **Which file decides is a question about the name alone, the way the shelf's and the inbox's are**: the shallowest outline basenamed `kolu.olai` (case folded, `_olai/Kolu.olai` the chosen form) — a file that holds no `watch` node still decides, so a root `Kolu.olai` of notes shadows one parked deeper, knobs defaulting until it moves. The watch node carries the knobs — `held-for` (the pause that holds a terminal's state out of the feed), `nag` (how often an unanswered one is said again) and `heartbeat` — in the grammar a `kolu watch` hand already types, because both faces read padi's own parser: the units are `ms`, `s`, `m`, `h` and `d`, and an absent value defaults. One difference from the CLI: a **bare number is refused** — the `kolu` flags take bare milliseconds because flags already mean that, but a file is not a flag: a truncated `nag: 10` must be said, not defaulted into a ten-millisecond spin. A malformed one defaults **and is said on the server's console at warning level**, so a typing mistake is never silent — and it is told in the face's own sentence, not a second spelling of it. `held-for` may be `0` — the report the INSTANT a state lands, which padi's watch flags also allow — but the other two never are, because a nag every 0 ms is the spin padi itself refuses.
