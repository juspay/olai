...
**A re-told wake says which telling it is.** When a nag rides the event, the body carries one more line after its attribution: *This is reminder 2 of 3.* — and on the last one: *…, the last — this doorbell goes quiet about this terminal until its state changes.* The first saying carries no count, because the cap the count counts against is padi's own repetition of it; the words are spelled off the event's accounting exactly as the events drawer spells them, so a count can never disagree with itself between the two places you read it.
