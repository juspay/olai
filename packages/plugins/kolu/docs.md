...
**Two nags in one window make one note.** That combining is over bodies still *waiting*; what it cannot reach is two that each land at once. Every nag re-says the *whole* standing set, and it runs per terminal on its own clock (the `nag` knob) — so two parked lanes whose clocks align by the minute can hand you one paragraph twice, and the second copy carried nothing the first did not (seen 2026-09-02, the orchestrator's conversation: one "2 terminals went quiet … a note, not a call" sentence, twice, one minute stamp). So the doorbell remembers, per conversation and per meaning, the set the last delivered note named: **a nag whose set was already said inside the nag window in force is coalesced away.** Three things that rule never eats: a `transition` — a fresh hold's first word, said once per hold, and a terminal that went back to work and went quiet again is news even where the sentence reads the same; a real change in the set — a lane joining or leaving, a state flip, a moved step — which rings inside whatever window it lands in; and the once-per-window reminder itself — a nag a full `nag` after the last note always rings, because the cadence is not the bug. Nothing else moves: the sentences say what they said, the knobs are what they were, and every nag still fires and still shows in the events drawer — only the second copy of one note is gone.
...
The moments are `event` (a watcher `transition`, `nag` or beat reached the doorbell), `derived` (one file's ringing set — **named**, not counted, because the fact worth reading is usually an *absence* and an absence is only legible against a list), `scopes`, `classified` (including `meaning=none`, which is the silence), `delivering` / `said` / `delivered` / `dropped`, `coalesced` (a nag whose set was already said inside the window — the delivery that did *not* happen, said out loud exactly like the others, with the file, the meaning and how far from the note it fell), and the beat's own `beat`, `beating`, `beat-passed`, `beat-said` and `beat-dropped`. Every `dropped` carries a `why` — and it is one word rather than two, because a set that has nobody standing is the same fact whether the ring asked before handing the delivery over or the closure asked at the moment the words would have gone in. The `no-revision` spelling is once per **file** per event — three seats on one board are one ask, so they read as one line — and a tick with no seats at all asks nothing of the vault and says nothing.
