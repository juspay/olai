...
**The watch's pacing lives in `_olai/Settings.olai`**, under kolu's row on the plugins panel:

```
{"id":"kolu","ord":"a0","title":"kolu","custom":{"plugin":"kolu","held-for":"60s","nag":"10m/4","heartbeat":"30m"}}
```

The file is normal outline records, one node per plugin. The `plugin` property is the row's word — a title is prose somebody renames, and the identity is not. The knobs are `held-for` (the pause that holds a terminal's state out of the feed), `nag` (how often an unanswered one is said again) and `heartbeat` — in the grammar a `kolu watch` hand already types, because both faces read padi's own parser: the units are `ms`, `s`, `m`, `h` and `d`, and an absent value defaults. One difference from the CLI: a **bare number is refused** — the `kolu` flags take bare milliseconds because flags already mean that, but a file is not a flag: a truncated `nag: 10` must be said, not defaulted into a ten-millisecond spin. A malformed one is refused on the panel, and a hand-edit that the schema cannot judge defaults **and is said on the server's console at warning level**. `held-for` may be `0` — the report the INSTANT a state lands, which padi's watch flags also allow — but the other two never are, because a nag every 0 ms is the spin padi itself refuses. They used to live on the `watch` node of `_olai/Kolu.olai`; that node is left alone and is not what the serve reads.
...
The file is yours, not installed: without a kolu node in `_olai/Settings.olai` the watch runs its defaults (sixty seconds, ten minutes, half an hour). Edits on the plugins panel land live: the watcher observes the change without a restart.

**There is no mute list any more, and the feed has no silence control at all now.** The file used to carry a `mutes` node whose children named terminals the watch was to keep quiet about, and the drawer's foot named them back. Both went with the second doorbell, and what went with them was the only way there ever was to quiet THIS surface: the watch says everything it sees — every held terminal in the fleet, for every reader of the page — and `_olai/Settings.olai` decides the pacing and nothing else.
