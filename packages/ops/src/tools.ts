 ...
 * It names `_olai/Trash.olai` files the set already serves and empties them whole,
...
export const TOOLS: ReadonlyArray<Tool> = [
  ...
  write(
    "trash_node",
    "Archive a subtree",
    "Move a node and everything under it into `_olai/Trash.olai` beside its outline, re-creating the chain of ancestor titles it hung off. Ids move with the nodes, so mirrors and edges pointing at them keep resolving. Nothing is stamped: archiving is not finishing. What DOES change beyond the file is which readings draw it: an archived node is out of every date reading — no day page, no calendar day, nothing owed on the agenda — and out of a search unless the query says `is:trashed`. Its own dates and marks are untouched, and `untrash_node` is the way back.",
    TrashRequest,
    { op: "trash" },
  ),
  write(
  ...
  write(
    "empty_trash",
    "Empty the trash",
    "PERMANENTLY DELETE every record in the archives you name. This is the only write in this whole surface that destroys rather than moves, and the only one nothing puts back: `trash_node` is a trash and `untrash_node` is the way out of it, and this is what stops carrying the pile. What survives is whatever git had already recorded — the files are rewritten with no records in them, through the same gate and the same commit door as every other write — so a directory with no history, or one whose archive was never committed, keeps nothing.\n\nIT NAMES FILES AND NEVER A NODE. There is no way here to delete ONE row out of the trash; the unit is the archive, exactly as a bin is emptied rather than picked through. NAME EVERY TRASH YOU MEAN IN ONE CALL (`list_outlines` says which files are `_olai/Trash.olai`; a directory can hold one beside each outline's directory). That is not a convenience: what may still point into an archive is judged against the UNION of the archives in this call, so a `see` from one of these piles into another is a record this write deletes rather than a holder that refuses it. Split the same emptying across two calls — or two `apply` entries — and the two piles refuse in one order, land in the other, and refuse both ways round when they name each other.\n\nREFUSED four ways, and the fourth is the one to plan around: an outline the set does not hold, an outline that is not an archive (nothing here deletes out of a live outline — `trash_node` is how a node leaves one), archives that hold nothing between them, and — the important one — a record OUTSIDE every archive named still pointing into one. Ids move with a node when it is archived, so a mirror, a `see` or an `after` written in a live outline goes on resolving at what was put away; deleting those records would leave it naming ids nothing declares. The refusal names each such record with its `file:line` and the field it points with. Re-point or retire them, or `untrash_node` what they name back out, and call again. The `.md` a `doc` field named is a FILE and is never touched.\n\n`was` IS THE COUNT YOU EXPECT, together, across those archives — optional, and worth sending whenever a number was shown to somebody. A write is re-planned against a newer snapshot when the store moves under it, and a re-plan of this one silently widens: a node archived in between goes too. With `was`, that is a refusal naming both counts instead.",
    EmptyRequest,
    { op: "empty" },
  ),
  write(
