...
export const asking = (
  ...
  // on `document` below, which is the other. It grew the arm when it grew the
...
export const TOOLS: ReadonlyArray<Tool> = [
  ...
  write(
    ...
    "Move a node and everything under it into `_olai/Trash.olai` — the one trash for the served directory — recording the outline it left as the outermost scaffold title, then the chain of ancestor titles it hung off. Ids move with the nodes, so mirrors and edges pointing at them keep resolving. Nothing is stamped: trashing is not finishing. What DOES change beyond the file is which readings draw it: a node put away is out of every date reading — no day page, no calendar day, nothing owed on the agenda — and out of a search unless the query says `is:trashed`. Its own dates and marks are untouched, and `untrash_node` is the way back.",
    TrashRequest,
    { op: "trash" },
  ),
  write(
