 ...
 * `_olai/Trash.org` the set already serves and empties it whole, so it is
...
export const TOOLS: ReadonlyArray<Tool> = [
  ...
  write(
    "empty_trash",
    "Empty the trash",
    "PERMANENTLY DELETE every record in `_olai/Trash.org`. This is one of the two writes in this whole surface that destroys rather than moves — `delete_file` destroys FILES; this one destroys RECORDS — and the record's node-and-all story is the pile's: `trash_node` puts a node away, `untrash_node` is the way out of it, and this is what stops carrying the pile. What survives is whatever git had already recorded — the file is rewritten with no records in it, through the same gate and the same commit door as every other write — so a directory with no history, or one whose trash was never committed, keeps nothing.\n\nIT NAMES THE TRASH AND NEVER A NODE. There is no way here to delete ONE row out of the trash; the unit is the pile, exactly as a bin is emptied rather than picked through. Name `_olai/Trash.org` (`list_outlines` says the files the set holds). A leftover `Archive.org` is not the trash and is refused.\n\nREFUSED four ways, and the fourth is the one to plan around: an outline the set does not hold, an outline that is not the trash (nothing here deletes out of a live outline — `trash_node` is how a node leaves one), a trash that holds nothing, and — the important one — a record OUTSIDE the trash still pointing into it. Ids move with a node when it is put away, so a mirror, a `see` or an `after` written in a live outline goes on resolving at what was put away; deleting those records would leave it naming ids nothing declares. A PROPERTY COUNTS TOO where the vault declared its key `ref` or `node` in `_olai/Properties.org` — such a value is a reference, and the refusal names it with the key as the field. The refusal names each such record with its `file:line` and the field it points with. Re-point or retire them, or `untrash_node` what they name back out, and call again. The `.md` a `doc` field named is a FILE and is never touched.\n\n`was` IS THE COUNT YOU EXPECT — optional, and worth sending whenever a number was shown to somebody. A write is re-planned against a newer snapshot when the store moves under it, and a re-plan of this one silently widens: a node put away in between goes too. With `was`, that is a refusal naming both counts instead.",
    EmptyRequest,
    { op: "empty" },
  ),
  write(
