/**
 * The node-agent door as a pure plan judgement.
 *
 * THE FORBIDDEN KEY IS A LITERAL HERE, and it has to be: `Door.forbidden` is
 * DATA on a ticket ({@link ./door.ts}) and the caller that fills it is a
 ...
 * carry and asserts what the door does with it, which is all this file was
 * ever about.
 */
import {
  NO_KINDS,
...
import { barred, type Door } from "./door.ts"
 ...
 *  word, and what it asserts is what the door DOES with whatever it carries. */
...
const asked = (request: Request, door: Door = kitchen) => {
  ...
  return barred(door, at.derived, planned.success)
}

describe("node-agent write door", () => {
  ...
  test("...and the approval one is refused on a node the agent otherwise owns", () => {
    ...
    // node. The door judges the RECORDS a plan changed, so all of them land on
   ...
   *  The door reads a MOVEMENT rather than a direction, so it is refused with
   *  the rest — and an asymmetric rule here would be one more thing a reader of
   *  the door has to hold for a capability nothing asks for. */
  test("...and taking one off is writing it, because the door reads a movement", () => {
  ...
  test("the refusal quotes the clause the door carried for that key", () => {
