...
// ── the filter over one page ───────────────────────────────────────────
//
// THE DIRECTORY-WIDE HALF IS NOT HERE ANY MORE. `search` — a query ranked and
// shortened, the reading behind `search_nodes` and behind every box a person
// types into — is `olai-plugin-search`'s `matcher.ts`, standing behind
// {@link ./ops.ts}'s `Search` door. It took the trigram table with it, which is
// what makes a serve minus that row a serve with no matcher rather than a serve
...
// asked as a standing view rather than as a call, and it never touched the
