...
doing, retitle, note, schedule, move, archive. Everything that changes an
outline goes through here, and everything an agent may READ of one comes out of
here too.
...
The last field edits are this format's own: the reference had no structural
move and no separate note edit. `move:` keeps its meaning for a date, which is
what it named there.
