...
{ pkgs }:
# THE SECOND STAGE: the plugin names the file. A favicon is neither a seed nor
# a package; it is one file at a known path in the same source the tenant
# already hydrates from, so it is read as one. `svg` is that file, `revision`
# the pin's revision, `from` a human sentence saying where it came from —
# `kit.mark { svg = "..."; revision = ...; from = "..."; }`.
{ svg, revision, from }:
let
