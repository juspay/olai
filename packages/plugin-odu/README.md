...
- **Kinds and an owned file.** Odu declares no property kind and owns no outline. `worktree` is a `path` the vault declares for its own reasons and this plugin reads it — which may stay true, and is not yet a decision.

The vault WALK that reads it has landed, though, and it is the sharpest thing in this package: [`src/worktrees.ts`](src/worktrees.ts) was `packages/server/src/worktrees.ts`, and it belongs here because what it decides is whether olai dials a socket in somebody's checkout. It may not live in `@olai/odu-client` — it reads outline records, and that package's interfaces are parametric in the node type so a compiler holds it to never learning what one is — and it has no business in core, which holds the vault but makes no judgement about odu. Between those two is what this package is.
