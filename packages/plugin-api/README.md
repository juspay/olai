...
| `settings` | `register(schema, options)` / `get` / `watch` / `update` / `unset` — a person's runtime configuration, stored in `_olai/Settings.olai`, stamped by the calling plugin's own word. Schema defaults, then the vault. Host facts stay on the row. A field annotated `secret` is refused at register; a `restart` field is badged pending |
