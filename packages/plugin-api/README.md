...
**ENGINES** are the ACP agents the chat panel can seat: `claude`, `codex`, `opencode`, `pi`, `omp`. Each is one directory and one row, because they share no release clock — the Claude adapter's pin moved five times in a month and opencode's has never moved — and because the file’s row selection then enables them one at a time: a policy selecting only codex, opencode, pi is a serve with no Claude row, no probe of one, and no mark for one anywhere. What an engine hands over is DATA and pure functions (`agents.register` below); it never spawns, sends or sees a transcript, which is `olai-plugin-chat`'s.
...
[`src/contract.ts`](src/contract.ts) is what both halves share — `PropKind`, `Probed`, `StdioServer`, `Deliveries`, `Wake`, `PluginWire`, and the word a kind is composed into. `Probed.missing` spells its three fields inline rather than importing the engine wall's `NotHere`: contravariance makes the narrower spelling the stronger claim, so an MCP, padi or mail probe may answer with anything carrying `name`, `where` and `why` — including an engine's own value — and this door owns no word it does not read. These contracts carry no live runtime, so neither process pays for the other's graph.

An engine registration lives at [`@olai/acp/engine`](../acp/README.md): `Leg`, `Adapter`, `Where`, `Registering` and `PromptChannel` describe an ACP agent and how to reach it. `Registering.at(where)` returns `Adapter | NotHere`, so the engine authors a precise absence reason at the probe rather than supplying a second browser-only sentence. Chat owns detection and publishes every mounted engine's standing; no live value crosses a package boundary by import.
...
| `localState` | `load` / `save(record)` — a small opaque document this plugin keeps about this serve, in the state home rather than the vault, minted from the calling plugin's own word |
| `agents` | `register(engine)` — one ACP engine this build can seat: a `Leg`, a probe answering `Adapter \| NotHere` for this host, and the standing-prompt channel. The ID is the fiber's word. The missing arm carries the engine's reason and optional installation link; chat publishes it beside installed engines rather than discarding it |
...
| `ops` | the narrow vault door plugins need: an opaque reading, a page read through core's standing cache, one property write, one document mint, and refused-write notices. It is not the ops layer and cannot expose its other verbs |
...
**A face hangs in a declared slot.** Capability owners publish their own static descriptors and TypeScript contracts; the permanent API supplies registration and withdrawal, not a central catalog. A registration is checked against the slot owner's contract:
...
| `delivery.mark` | plugin | the shapes drawn over a sentence this plugin delivered into somebody's conversation — a `<g>` in a sixteen-unit box, never a whole `<svg>` |
...
`olai-plugin-chat` declares the shape of the question and each plugin declares its own answer, and **neither imports this package**: a plugin may not (the registry imports every plugin), and chat is a general package a floor below the plugin system that is handed a list. The two spellings meet in one expression at the composition root, where a drift between them is a type error.
...
What core does with a body is three arms, and which one it took is never reported back — there is no arm a plugin would answer differently. A conversation whose agent is **idle** takes it as a turn of its own, on a row marked with the plugin's name (`@olai/surface`'s `rang`, stamped by core from the registry binding, never by the caller). A conversation whose agent is **mid-turn** HOLDS it until the turn boundary, because a message that arrived alongside a running turn would spend the human's interruption with nobody having typed anything ([`olai-plugin-chat`](../chat/README.md)'s `queuedHere`). A conversation **nobody is in** holds it until somebody opens it. Bodies sharing a `coalesce` key replace each other in place while they are still held, which is what lets a plugin send a fresh whole sentence per event and have a person read one message rather than five.
...
Pass the complete recipient returned by `scopes()` or `ringing()` to `deliver(scope, body)`. Its `current()` capability preserves the exact saved choice and Cordis consumer activation through asynchronous work. Clearing, repicking (including the same file), eviction, and activation teardown revoke old recipients; the chat checks them immediately before writing a transcript entry. Fault marking and healing preserve choice identity. A choice changes only after its save succeeds.

Delivery-only plugins retain derived node recipients. Their direct conversation notices, such as a transport failure, use `notify({ agent, session }, body)`; plugins declaring a selectable wake cannot use this route to bypass the picker. Both methods stop accepting work when the consumer's Cordis activation leaves.
...
## One LocalState document, and core does not interpret it
...
A browser plugin’s `outline.row.action` callback may return a string when a row action is refused. Core displays that sentence beside the row in the menu’s alarm style. Return nothing for success; do not discard a surface procedure’s failure result.
