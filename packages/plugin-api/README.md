...
[`src/contract.ts`](src/contract.ts) is what both halves share — `PropKind`, `Probed`, `StdioServer`, `Deliveries`, `Wake`, `PluginWire`, and the word a kind is composed into. It re-exports the `NotHere` data shape owned by `@olai/acp/engine`. These contracts carry no live runtime, so neither process pays for the other's graph.
