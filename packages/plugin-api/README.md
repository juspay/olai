# @olai/plugin-api — the only place core meets an appliance
...
That residue is not sloppiness. It is the part that genuinely was **olai's own judgement about an appliance** — what an absent padi means, which vault file is kolu's by convention, which property wears which face. What was missing is a place to put a judgement about an appliance that is neither the appliance nor the core. **This is that place, and it is the only one.** The tenants that stand on it — [`olai-plugin-kolu`](../plugins/olai-plugin-kolu/README.md) and [`olai-plugin-odu`](../plugins/olai-plugin-odu/README.md) — live one directory over, in `packages/plugins/`, and `@olai/kolu-ui` is not a package any more: the appliance fold made it `src/ui/` inside the kolu tenant, so an appliance is TWO packages (its dial, and everything olai says and draws about it) rather than three.
...
**AND THE SCOPE ITSELF CAN BREAK, so the plugin declares the sentence for that too.** `PluginServerHalf.wake` carries a fourth string beside the three the strip draws: `gone`, the whole sentence a conversation is told when the file it woke on stops being served. The defect it closes is a silence — a doorbell over a renamed file derives nothing, forever, and quiet-because-broken looks exactly like quiet-and-fine on every channel there is. Core DETECTS it, because core owns the served set and the picks and the plugin owns neither once its file has gone ([`@olai/chat`](../chat/README.md)'s `Chat.faults`), and core SAYS nothing: it carries this string through the same door, whole, naming no file and joining nothing to it. That is why it is one string where the other three are pieces — nothing is drawn between its halves. It is REQUIRED wherever `wake` is present, because a plugin that rings has scoped conversations and every one of them can be renamed out from under it. A scope in that state also leaves `scopes()` entirely, so a doorbell cannot ring for it and nothing else a plugin does per scope can either.
...
`@olai/plugin-api` imports every plugin. **No plugin imports it back** — a plugin declares its manifest as a plain `as const` object and the fit is proved here, by `satisfies`, which is the same structural agreement `@olai/ops` keeps with the surface's `Status`. So the dependency is a DAG the manifests express, and a cycle is not a rule a reviewer remembers but a thing `bun install` cannot describe.
