...
| `settings` | `register(schema, base, options)` / `get` / `watch` / `update` / `unset` — a person's runtime configuration, stored in `_olai/Settings.olai`, stamped by the calling plugin's own word. Schema defaults, then the row's config, then the vault. A secret never reaches a page; a `restart` field is badged pending |
