...
test("concurrent callers carry independent authority outside untrusted input", async () => {
  ...
  const alice = authorityAt(bound, { writer: "alice", door: "session-a" })
  const bob = authorityAt(bound, { writer: "bob", door: "session-b" })
  expect(alice.read).toBe(read)
  expect(bob.read).toBe(read)
  const forged = { writer: "admin", door: null }
  ...
  expect(results).toEqual([
    { writer: "alice", door: "session-a" },
    { writer: "bob", door: "session-b" },
    [{ writer: "alice", door: "session-a" }],
  ])
