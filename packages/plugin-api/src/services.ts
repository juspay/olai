 ...
 * sixth is the search row's matcher. Every other service on this page is a fact
 * about the process, the vault or the machine, which core knows before any row
 * is mounted — so there is nothing a row could offer that core is not already a
 * better answer for, and everything to lose by letting one try.
 */
export const OFFERABLE = [Agents, Deliveries, SessionStart, Watching, Ledger, Search] as const
...
export interface Plugins {
   ...
   *
   * ONLY THE SIX OFFERABLE ONES ever appear: core's own services are provided
...
export const openPlugins = (
  ...
  Effect.gen(function*() {
     ...
     * the git row's and {@link Search} is the search row's. Offered from the
