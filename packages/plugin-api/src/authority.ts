 ...
 * Providers interpret their own session rule. The host only carries the value
 * through the call's Effect context; it does not replace provider handlers. */
import type { SurfaceHandlers } from "@kolu/surface/server"
import { Context, Effect, Stream } from "effect"
export const RequestAuthority = Context.Reference<{ readonly writer: string, readonly rule?: unknown }>("olai/RequestAuthority", {
...
export const authorityAt = (
  ...
  caller: { readonly writer: string; readonly rule?: unknown },
): SurfaceHandlers => {
