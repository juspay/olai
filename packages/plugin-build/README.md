...
**Binds** (`src/bind.ts`). `import { createSignal } from "solid-js"` becomes `const { createSignal } = globalThis.__olai_plugin_modules["solid-js"]`. Bundling the dependency in would give a second Solid — a second reactive runtime, whose components cannot read the app's contexts — and leaving it external would leave a bare specifier nothing resolves. So the specifier is never resolved anywhere: it is replaced by a read of a table the host filled from its own static imports, which is the only arrangement in which *the same copy* is true by construction. The statement is READ FORWARD once rather than matched with one expression, because an expression that admits a lazy whitespace class after a `\s+` is quadratic on a long run of spaces and the input here is somebody's own source (CodeQL `js/polynomial-redos`, on the PR that added this). And the rewrite does not trust its own reading — it asserts afterwards that no module syntax is left, and refuses in words naming the line if any is.
