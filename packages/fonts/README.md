...
package is that table plus the two things derived from it, and the reason all
three are here is that they were three different answers to one question in
three different places — a nix file, a build script and a client directory —
and only one of them could be read at a time.

| file | what it owns |
|---|---|
| `src/catalog.ts` | the table: every typeface a person can pick (name, label, group, hint, and the three stacks), and every FILE the picks between them need |
| `src/css.ts` | the sheet that table becomes — one `@font-face` per hosted file, one `:root[data-font="…"]` block per row |
...
1. A row in `TABLE` (`src/catalog.ts`).
2. If it is hosted rather than generic: its files in `HOSTED_FILES`, and the
   nixpkgs paths for those same files in `default.nix`.
...
What is left in `@olai/web`'s build is a **copy**, one catalog entry at a time
out of `OLAI_FONTS_DIR` (`shell.nix` and the root `default.nix` both point at
this derivation). The lookup stays by name rather than copying the directory
wholesale, because the catalog is what the generated sheet asks for: a face it
names that the derivation does not convert has to fail the build rather than
...
No dependencies at all — a table and two pure functions over it. Nothing here
fails, does IO, or knows what a browser or a build is; the one thing that
