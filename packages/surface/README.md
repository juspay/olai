...
Tests here pin the permanent management member set. The bundle's `application-surface.test.ts` checks preserved member tags, read-only declarations, collection semantics and schema array keys across the capability-owned descriptors, and its `published.test.ts` and `published.equivalence.test.ts` hold the three rows' projections up together — a claim about three rows at once is a claim only the registry can make.
