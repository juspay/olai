...
`./client` builds a typed face from a supplied surface and dispatch. It has no fixed application spec, and there is no longer a package that gives it one. `@olai/bundle/surface` and `@olai/bundle/client` were a flat aggregate of every row's members under bare names plus a typed client over it; #546 deleted both, because a member has one tag now and it carries its owner, so the aggregate described a wire nothing serves. There is no flat contract left anywhere: `olai-plugin-mcp` held one for as long as `serveSurfaceAsMcp` took a single surface and built every `surface://` URI out of that spec's member keys, and juspay/kolu#2234 replaced it with a rooted bundle — a bare core beside a keyed set of siblings, each resolved against its own spec — so an agent's URIs now name the row that answers them (`surface://collections/markdown/documents`). Browser capabilities consume their own scoped sibling clients, supplied by `Wired`, rather than a permanent full-application client.
