...
const documentsOf = (
  ...
  held: Pick<Published, "documents"> | null,
...
/** A capability retains its own values and only the shared file-name census
 * needed to distinguish incremental updates from a resync. It never retains
 * another capability's bodies or records. */
export interface Projection<T> {
  readonly files: ReadonlySet<string>
  readonly change: Change<T>
}
const frame = (snapshot: Snapshot<Reading>, previous?: ReadonlySet<string>) => {
  const decoded = snapshot.changed.flatMap(path => {
