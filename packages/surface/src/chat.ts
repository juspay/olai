 ...
 *     picker draws, and `attach` — the one that carries BYTES, in bounded
 *     chunks, because a pasted picture is the one thing about a conversation
 *     that is not already a string. Each declares its failure channel, so "a
 *     turn is already running" arrives as a `busy` a caller can branch on
...
import { BusyFailure, isOpFailure, kindOf, OpFailure, Unfinished, UsageFailure } from "@olai/format"
...
export { BusyFailure, isOpFailure, kindOf, OpFailure, Unfinished, UsageFailure }
