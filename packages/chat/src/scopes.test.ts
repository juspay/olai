...
describe("a pick, across a restart", () => {
  ...
  test("a second pick for the same doorbell replaces the first", async () => {
    const scopes = await run(forDirectory(HERE))
    await run(scopes.set(IN, "kolu", "Fleet.org"))
    await run(scopes.set(IN, "kolu", "Other.org"))
    expect(scopes.rows()).toEqual([
      { agent: "claude", session: "sess-1", plugin: "kolu", file: "Other.org" },
    ])
  })
  ...
  test("two plugins in one conversation are two picks", async () => {
    const scopes = await run(forDirectory(HERE))
    await run(scopes.set(IN, "kolu", "Fleet.org"))
    await run(scopes.set(IN, "odu", "Runs.org"))
    // The triple's middle column is what makes a per-plugin door answerable at
    // all: clearing one must not clear the other.
    await run(scopes.set(IN, "kolu", null))
    expect(scopes.rows()).toEqual([
      { agent: "claude", session: "sess-1", plugin: "odu", file: "Runs.org" },
    ])
  })

  test("the same session id under another agent is another conversation", async () => {
    const scopes = await run(forDirectory(HERE))
    await run(scopes.set({ agent: "claude", session: "sess-1" }, "kolu", "A.org"))
    await run(scopes.set({ agent: "opencode", session: "sess-1" }, "kolu", "B.org"))
    expect(scopes.rows()).toEqual([
      { agent: "claude", session: "sess-1", plugin: "kolu", file: "A.org" },
      { agent: "opencode", session: "sess-1", plugin: "kolu", file: "B.org" },
    ])
  })

  test("another directory's picks are not this one's", async () => {
    await run((await run(forDirectory(HERE))).set(IN, "kolu", "Fleet.org"))
  ...
  test("nothing is written under the served directory", async () => {
    ...
    try {
      await run((await run(forDirectory(served))).set(IN, "kolu", "Fleet.org"))
...
describe("two picks at once", () => {
  test("both land, rather than one racing the other's staging file", async () => {
    ...
    const both = await Promise.all([
      outcome(scopes.set(IN, "kolu", "Fleet.org")),
      outcome(scopes.set(IN, "odu", "Runs.org")),
    ])
...
describe("the cap", () => {
  test("the least recently touched pick goes, and only it", async () => {
    ...
    for (let n = 0; n <= ROWS; n++) {
      await run(scopes.set({ agent: "claude", session: `sess-${n}` }, "kolu", "F.org"))
    }
  ...
  test("touching a pick again keeps it, whatever its position", async () => {
    const scopes = await run(forDirectory(HERE))
    await run(scopes.set({ agent: "claude", session: "old" }, "kolu", "F.org"))
    ...
    await run(scopes.set({ agent: "claude", session: "old" }, "kolu", "G.org"))
    // ... so the next pick evicts, and what it takes is the row `old` was in
    // front of rather than `old`.
    await run(scopes.set({ agent: "claude", session: "fresh" }, "kolu", "H.org"))
...
describe("a record that cannot be trusted", () => {
  ...
  test("a damaged ROW is dropped and the rest still open their doorbells", async () => {
    ...
    damage(JSON.stringify({
      ...
      scopes: [
        ...
        { agent: "claude", session: "sess-2", plugin: "odu", file: "Runs.org" },
        7,
      ],
    }))
    expect((await run(forDirectory(HERE))).rows()).toEqual([
  ...
  test("a record about another directory is not this one's picks", async () => {
    await run((await run(forDirectory(HERE))).set(IN, "kolu", "Fleet.org"))
    damage(JSON.stringify({ cwd: ELSEWHERE, scopes: [{ ...IN, plugin: "kolu", file: "X.org" }] }))
    expect((await run(forDirectory(HERE))).rows()).toEqual([])
  })
  ...
  test("a state directory that will not take a write refuses OUT LOUD", async () => {
    ...
    try {
      expect(Result.isFailure(await outcome(scopes.set(IN, "kolu", "Fleet.org")))).toBe(true)
    } finally {
  ...
  test("... and leaves the mirror where the person was told it stayed", async () => {
    ...
    try {
      expect(Result.isFailure(await outcome(scopes.set(IN, "kolu", "Other.org")))).toBe(true)
    } finally {
      chmodSync(home(), 0o700)
    }
    expect(scopes.rows().map((row) => row.file)).toEqual(["Fleet.org"])
  })
})
...
describe("what a write says it removed", () => {
  test("a write answers with the rows it removed, so a caller can take their doorbells back", async () => {
    ...
    expect(await run(scopes.set(IN, "kolu", "Fleet.org"))).toEqual([])
    // Re-pointing removes the row it replaced — the caller holds bodies derived
    // from the OLD file and has to hear that it is gone.
    const moved = await run(scopes.set(IN, "kolu", "Other.org"))
    expect(moved.map((row) => row.file)).toEqual(["Fleet.org"])
    // ... and so does a clear.
    const cleared = await run(scopes.set(IN, "kolu", null))
    expect(cleared.map((row) => row.file)).toEqual(["Other.org"])
  })
  ...
  test("an EVICTED row is reported too, which is the one nobody made a gesture about", async () => {
    ...
    for (let n = 0; n < ROWS; n++) {
      await run(scopes.set({ agent: "claude", session: `sess-${n}` }, "kolu", `File-${n}.org`))
    }
    const left = await run(scopes.set({ agent: "claude", session: "sess-new" }, "kolu", "New.org"))
...
describe("a pick its doorbell cannot watch", () => {
  ...
  test("the edge is answered once, and the mark is on the disk", async () => {
    ...
    expect(fell.map((row) => row.file)).toEqual(["Fleet.org"])
  ...
  test("... and it can break again afterwards, which is a second thing that happened", async () => {
    ...
    expect((await run(scopes.faults(RENAMED, TELLABLE))).map((row) => row.file)).toEqual(["Fleet.org"])
  })
  ...
  test("a re-pick clears the mark, so the new file gets its own fault", async () => {
    ...
    expect(
      (await run(scopes.faults((_plugin, file) => file === "Other.org" ? "gone" : null, () => true)))
        .map((row) => row.file),
    ).toEqual(["Other.org"])
  })
  ...
  test("a fault is not a TOUCH, so it does not move a row's place in the eviction order", async () => {
    ...
    const scopes = await run(forDirectory(HERE))
    await run(scopes.set({ agent: "claude", session: "a" }, "kolu", "Fleet.org"))
    await run(scopes.set({ agent: "claude", session: "b" }, "kolu", "Other.org"))
    await run(scopes.faults(RENAMED, TELLABLE))
  ...
  test("a mark that will not parse reads as an unmarked row, and the pick survives", async () => {
    ...
    const restarted = await run(forDirectory(HERE))
    expect(restarted.rows().map((row) => row.file)).toEqual(["Fleet.org"])
