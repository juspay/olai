...
## The registry is a row, and the browser's two are still source files
...
| --- | --- | --- |
| `./wire` | [`@olai/web`](../web/README.md)'s `client/wire.ts`, and `@olai/server`'s `pluginPolicy.ts` for the one question a flag asks before anything mounts | each plugin's own `./wire` — the sibling map and the filters — and it stops there |
| `./bundle` | [`@olai/server`](../server/README.md)'s composition root | the rows, the loader, the `--plugins` patch and the resolver. NO plugin's code is on this graph: a row's `name` is a specifier the loader resolves at mount |
| `.` | the browser: `client/plugins/` mounts each tab half, hangs each chrome readout, and registers each dressing | the manifests whole — dressings, chrome, mounts, marks, which are SolidJS components and, behind kolu's, a terminal emulator |
