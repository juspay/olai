...
| `Slots.register` | WHERE A FACE HANGS: six declared slots, keyed by the plugin (`app.header`, `app.mount`, `chat.speaker.mark`) or by a property KIND (`outline.row.chip`, `.pane`, `.block`). Each registration is a finalizer on the plugin's scope, so a plugin the roster stops naming unwinds its own faces | `./browser` |
...
[`olai.yml`](olai.yml) is the SERVER's list: one row per plugin, an `id` and the module the loader mounts. It is DATA, and that is what makes `--plugins` a `disabled` **patch** over rows — through the loader's own overlay mechanism — rather than a `.filter` somewhere in code. [`src/bundle.ts`](src/bundle.ts) mounts it through [`@olai/effect-cordis/loader`](../effect-cordis/README.md) and supplies the module-resolution seam the loader needs, because a bare `import()` from inside the loader's own package cannot see a workspace member under the isolated linker.
...
`reportBundle(host)` is what the panel's row is written out of, and it is here rather than in the composition root because it reads the ROWS. A plugin has a lifecycle, so "not running" is four different mornings — the loader declined to load the row, its `apply` failed, it is short of a service it names, or it loaded and registered nothing — and `running: false` on the wire said only that one of them happened. The reading walks the live registry rather than the loader's entries, and it is an Effect because a failed plugin's error is PRIVATE and reachable only by asking for it; that message crosses to the panel verbatim, because the failure prose is the plugin's and core's job is to carry it. Both readings are [`@olai/effect-cordis`](../effect-cordis/README.md)'s — this package has never heard of the engine. What this reading deliberately cannot answer is `optIn` — the row's own `disabled` and the flag's patch are the same field by the time the loader sees them, which is exactly what makes the patch a patch, so `@olai/server`'s `rosterOf` splits `off` from `optIn` off `--plugins` and nothing here re-reads the flag.
