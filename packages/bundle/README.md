...
Olai is a bundle of plugins, and this package is the list of which. Thirty-two rows in [`olai.yml`](olai.yml), and nothing here tells one kind from another: same shape, same doors, same row. What the file's own headings group them into is the CONVERSATION (`chat`, first, because every row below names one of its doors); the DIRECTORY (`vault`), the LEDGER (`git`), the MATCHER (`search`), WHO IS LOOKING (`identity`) and the JOURNAL, each standing behind a door core defines; the ENGINES, the ACP agents the chat panel can seat (`claude`, `codex`, `opencode`, `pi`, `omp`, in the order the picker draws them); the TENANTS — olai's judgement about somebody else's appliance — [kolu](https://kolu.dev), which runs coding agents in terminals and serves them to an agent over MCP, [odu](https://github.com/juspay/odu), which runs CI, and Xyne Spaces, which is off unless its node in `_olai/Settings.olai` says `on: yes`; the TRANSPORTS (`ws`, `mcp`, `web-app`), which are ordinary rows and not modes of core; the SHELL the tab draws with (`ui-renderer`, `layout`, `sidebar`, `preferences`, `theme`, `plugin-inspector`); the CONTENT and the ways through it (`navigation`, `outlines`, `markdown`, `files`, `pins`, `capture`, `trash`); `vault-plugins`, which mounts what the served directory itself defines; and two maintained acceptance fixtures (`test-layout`, `test-counter`) that share this catalogue so a suite exercises the real resolver rather than a bundle of its own.

The tenants came first and the paragraph below is their history. Both were extracted into packages of their own once — `@olai/kolu-client`, `@olai/kolu-ui` and `@olai/odu-client` — and both left a residue behind in the packages that are supposed to know nothing about them: a `kolu.ts` in `olai-plugin-chat`, a `koluConfig.ts` and a `claimants.ts` in `@olai/server`, a named `wiring.kolu` slot with a `koluHalf(…)` call beside it, four `...koluMembers` spreads in the middle of the wire spec, a row per plugin member in the server's expose map, a `padi/` folder in `@olai/web`, and one property key spelled at seven sites across four packages.

That residue is not sloppiness. It is the part that genuinely was **olai's own judgement about an appliance** — what an absent padi means, which vault file is kolu's by convention, which property wears which face. What was missing is a place to put a judgement about an appliance that is neither the appliance nor the core. **`packages/plugin-api/` is that place** — the INTERFACE a plugin is written against — and **this package is the list of which plugins there are**. The two were one for several rounds and split when a server half started importing the interface for the services it names, and a package that both names every plugin and is named by every plugin is a cycle the manifests cannot express. The tenants that stand on both — [`olai-plugin-kolu`](../plugins/kolu/README.md) and [`olai-plugin-odu`](../plugins/odu/README.md) — live one directory over, in `packages/plugins/`, and `@olai/kolu-ui` is not a package any more: the appliance fold made it `src/appliance/` inside the kolu tenant, and the dials folded in after it, so an appliance is ONE package (its `./appliance` door, and everything olai says and draws about it) rather than three.

## A plugin is two halves

BOTH HALVES ARE EFFECTS — a `name`, a `needs` list and an `apply` that is one Effect — named by one row in [`olai.yml`](olai.yml): the server's is mounted by the loader, the browser's is fetched as its own chunk when the roster names it. The browser half used to be a VALUE, an `OlaiPlugin` manifest listed in a compiled-in registry. The object could not survive the tab following the roster: a manifest is present whether or not the serve composed the plugin, so every walk over it carried a LICENCE beside it — and the two licences pointed opposite ways (a face drawn early and taken away is a flicker; a subscription opened early latches a `degraded` readout for the life of the page). A plugin the roster never named registers nothing, so there is nothing left to license. Between them a plugin contributes:

| | what it is | where |
| --- | --- | --- |
| `name` | the namespace, the preferences row, the docs page's address, the row's `id`, the fiber's name, and the settings namespace — one spelling | both |
| `surface` | a **whole surface of its own**, declared in its own package with its own member names — core composes it as a SIBLING under `name`. Absent on an ENGINE, which composes none: what an engine contributes to a tab already travels on the chat cell, which is core.s | `./wire` |
| `faces` | which face may see which of its members, its own `ExposeMap` per face, written against its own spec | `./wire` |
| `needs` | the services this plugin's server half names. The runtime holds it `waiting` until they exist, and the compiler computes `apply`'s requirements from the same list | `./server` |
| `apply` | one Effect, where every registration below is made and where the appliance's client is called. Each one is an `Effect.acquireRelease` on the plugin's own scope | `./server` |
| `Kinds.register` | property KINDS the vault may declare, handed to [`@olai/format`](../format/README.md) as data — the format imports no plugin. The word is composed from the plugin's own name | `./server` |
| `Surfaces.register` | the sibling, its faces, the deps that implement it, and an optional hand-back for its own write face | `./server` |
| `Wakes.register` | what the strip's doorbell control says, and the two sentences a broken scope is owed | `./server` |
| `Agents.register` | one ACP engine this build can seat: its `Leg`, a probe answering `Adapter \| NotHere`, and its standing-prompt channel. The id is the fiber's word. Chat owns detection and publishes all mounted engines, including their absence reasons | `./server` |
...
| `Slots.register` | faces in capability-owned locations, keyed by plugin or property kind. Each registration is a finalizer on the plugin's scope, so removing the plugin withdraws its faces. Engine installation reasons travel on chat's roster, not in a second slot | `./browser` |
...
[`olai.yml`](olai.yml) is the SERVER's list: one row per plugin, an `id` and the module the loader mounts. It is DATA, and that is what makes the file’s row selection a `disabled` **patch** over rows — through the loader's own overlay mechanism — rather than a `.filter` somewhere in code. [`src/bundle.ts`](src/bundle.ts) mounts it through [`@olai/effect-cordis/loader`](../effect-cordis/README.md) and supplies the module-resolution seam the loader needs, because a bare `import()` from inside the loader's own package cannot see a workspace member under the isolated linker.
...
| --- | --- | --- |
| `.` | [`@olai/web`](../web/README.md)'s `client/wire.ts`, which loads the chunks the roster names, dials them and composes them, and its `client/main.tsx` and `src/build.ts` beside it; `@olai/server`'s `configuration.ts` and `serve.ts` for policy reconciliation | the ROWS — an `id` and a thunk each — the names off them, and `bundleRank`, which is that list read as an ORDER. A person reads three of these lists (the engines a picker offers, the install rows under them, the servers a session reports) and a list that reshuffles between boots is a list nobody can read twice, so the order is imposed from here rather than taken from whichever `import()` resolved first. NOBODY WHO NEEDS IT IMPORTS IT: a plugin may not import the registry back, so the rank is HANDED IN at each end — `main.tsx` passes it with the mount (`attachRenderer(root, bundleRank)`, which reaches the renderer's facade on `BrowserMount.rank`, so a PLUGIN reading a slot through `Faces` gets the file's order rather than arrival order), `serve.ts` provides it as `@olai/plugin-api`'s `Bundle` service — and what a row may know about the list stays its own position in it. NO plugin's code is on this graph: the thunk's specifier is a string until somebody calls it, so a face, the SolidJS it is written in and the terminal emulator behind kolu's are all on a chunk this door merely NAMES. It used to be `./wire` and `.` — the browser-safe wire half and the manifests whole — and they collapsed into one because the graph got EMPTIER |
| `./bundle` | [`@olai/server`](../server/README.md)'s composition root | the rows, the loader, the file’s enablement policy patch and the resolver. NO plugin's code is on this graph either: a row's `name` is a specifier the loader resolves at mount |
...
One door for all of them would put a component on the graph of a process that renders nothing and a daemon's whole contract on the browser's, which is exactly what [`olai-plugin-kolu/appliance`](../plugins/kolu/src/client/README.md)'s own fence exists to prevent one floor down. [`src/fence.test.ts`](src/fence.test.ts) walks each closure rather than trusting the table.
...
The shapes both lists are written to are still declared HERE — `BundleRow` and `BrowserRow` in [`src/rows.ts`](src/rows.ts), which the generated modules import and are typed by, so a row the generator emits wrongly is a `tsc` error rather than a chunk that fails to mount. That is the same structural agreement `@olai/ops` keeps with the surface's `Status`.
...
the file’s row selection is a `disabled` PATCH over the rows, applied on the way in, so a row that is off never mounts: no sibling surface, no tag, no handler, no expose row, no `surface/<name>/` on the wire at all. A plugin that is off never probes, unmounts its chrome, registers no dressing, validates its kinds as plain text, and leaves the outline it would have owned an ordinary outline.

What is new is the *at every moment*. It used to be true because every composition door takes a plain keyed object of surfaces, so the filter ran once and nothing could move afterwards. It is true for a stronger reason now: every registration a plugin makes is an EFFECT with its own undo, so a plugin that is disposed drops its sibling, its kinds, its wake declaration and its listeners in reverse — and a plugin that lands in `failed` because its `apply` died installed none of them, with its siblings untouched.

The settings reader publishes the vault's row policy. The root patches loader
entries, and the panel writes the same file through the ordinary write door.
A durable switch survives restart and is visible in the ledger; the providers
needed to restore this door use explicit session switches.

`reportBundle(host)` is what the panel's row is written out of, and it is here rather than in the composition root because it reads the ROWS. A plugin has a lifecycle, so "not running" is four different mornings — the loader declined to load the row, its `apply` failed, it is short of a service it names, or it loaded and registered nothing — and `running: false` on the wire said only that one of them happened. The reading walks the live registry rather than the loader's entries, and it is an Effect because a failed plugin's error is PRIVATE and reachable only by asking for it; that message crosses to the panel verbatim, because the failure prose is the plugin's and core's job is to carry it. Both readings are [`@olai/effect-cordis`](../effect-cordis/README.md)'s — this package has never heard of the engine. What this reading deliberately cannot answer is `optIn` — the row's own `disabled` and the vault patch are the same field by the time the loader sees them, which is exactly what makes the patch a patch, so `@olai/server`'s `rosterOf` splits `off` from `optIn` from build defaults and settings readings; nothing here interprets policy provenance.
...
- [docs/architecture/plugin-system.md](../../docs/architecture/plugin-system.md) — the same subject as a tour.
