...
test("profiles select only catalogue rows and preserve build defaults", () => {
  for (const profile of ["web", "surface", "test-minimal"]) {
    ...
    expect<ReadonlyArray<string>>(on).toEqual(profile === "web" ? DEFAULT_BUNDLE_NAMES : profile === "surface" ? ["vault", "settings", "mcp", "outlines", "files", "pins", "capture", "trash", "vault-plugins", "outline-olai", "outline-org", "markdown", "hypertext", "csv", "image", "pdf"] : ["vault", "settings", "outline-olai"])
  }
})
