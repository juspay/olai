...
describe("only the registry knows a plugin's name in CODE, too", () => {
  ...
  const NOT_A_PLUGIN: Readonly<Record<string, ReadonlyArray<string>>> = {
    // The new kind rows share the application name, media vocabulary, and the
    // existing static kind readers. Exact entries keep this additive phase
    // honest; the reader/face moves remove their former owners from this list.
    "olai": [
      "acp/src/engine.ts",
      ...
      "plugins/markdown/src/server/bodies.ts",
      ...
      "plugins/outlines/src/browser/Nothing.tsx",
