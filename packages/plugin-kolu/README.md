...
- **Kolu's MARK** (`Mark.tsx`) — the face over a sentence the doorbell delivered into somebody's conversation. The chat panel names the speaker of every run of messages (you, the agent, or a plugin) and looks a plugin's face up by the name core stamped on the row, so this shape is contributed here and core keeps no table of them. Two overlapping panes with a prompt in the near one: it names the FLEET rather than the doorbell, because the doorbell is one door kolu speaks through and the reader's question is which appliance is talking.
