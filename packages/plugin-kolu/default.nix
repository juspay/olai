...
pkgs.runCommand "olai-plugin-kolu-mark"
{
  ...
  test -f "$svg" || {
    echo "kolu's mark is not at $svg — it comes from the npins kolu pin (npins/sources.json); update packages/plugin-kolu/default.nix if kolu moved packages/client/favicon.svg" >&2
    exit 1
  }
