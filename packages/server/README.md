...
| `edit.ts` | what a KEYSTROKE meant, in terms of ops: the browser sends an intent (`indent`, `toggle`, `a new sibling after this`) and this resolves the placement against the snapshot and hands back one request. It answers a second question about the same reading — what would TAKE THE WRITE BACK, which is where the row sat and which mark it carried, recorded for the browser's undo stack because those facts stop being true the moment the write lands. Pure over a reading, like the planner it feeds |
