...
| `agree.ts` | one type, and the reason this package needs it: where two packages that may not import each other declare one thing twice, the pair cannot be deduped but it can be CHECKED — as identity (Effect's own `Types.EqualsWith`), because assignability is exactly what lets such a pair drift, and carrying a SENTENCE so the compiler says what broke rather than "'false' does not satisfy 'true'" |
| `search.ts` | the palette's door to `@olai/ops`' `Query.search` — one call, and the FENCE that makes "one search, two faces" checkable. The question and the answer are spelled twice, once as the wire spec a browser compiles against and once as the reader the ops layer answers with, and neither package may import the other; this is the only module that sees both, so it asserts them identical as TYPES. Assignability would not do it: an extra field on a hit is assignable in both directions, so it reaches an agent verbatim and is dropped by the browser's encoder, silently. Pure over a reading, like its two neighbours |
