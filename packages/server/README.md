...
| `recall/recall.ts` | the semantic index behind `Query.searchWith` — ops' `Recall` seam implemented: an in-memory map of node id → vector, reconciled incrementally (by content hash) off the store's snapshot stream, `nearest` answering by cosine over it. No embedder found means `null`, which IS the substring-only degradation, and an embedder that dies answers empty rather than failing a search |
| `recall/embedder.ts` | how a vector is made, how a maker is FOUND, and WHEN this process may dial at all: the `Embedder` seam (a test's fake, a future API embedder), the one detection shipped (Ollama on loopback, `nomic-embed-text` by default, `OLLAMA_HOST`/`OLAI_EMBED_MODEL` honoured, absence answered as `null` and never as an error), and `embedderFrom(env)` — the resolution the composition roots state out loud, with `OLAI_EMBED=off` skipping the probe entirely. `Recall.open` takes the seam as a REQUIRED option so a network call cannot arrive by omission; every test lane sets the switch, and `embedder.test.ts` pins both halves (off dials nothing; unset does dial, so the switch is load-bearing) |
| `recall/cache.ts` | where the index sleeps and the argument for WHERE: `$XDG_CACHE_HOME/olai/recall/`, keyed by served path, never inside the served directory (it would sit in pending commits and wake the watcher); JSONL with the embedder id stamped on top, discarded whole on any doubt because a cache's honest recovery is a rebuild |
