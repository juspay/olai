...
| `recall/embedder.ts` | how a vector is made and how a maker is FOUND: the `Embedder` seam (a test's fake, a future API embedder), and the one detection shipped — Ollama on loopback, `nomic-embed-text` by default, `OLLAMA_HOST`/`OLAI_EMBED_MODEL` honoured, absence answered as `null` and never as an error |
