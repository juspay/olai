...
| `agree.ts` | two lines of types, and the reason this package needs them: where two packages that may not import each other declare one thing twice, the pair cannot be deduped but it can be CHECKED — as identity, because assignability is exactly what lets such a pair drift |
