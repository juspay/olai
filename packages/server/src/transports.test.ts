...
test("transport registrations release and restore browser assets, sockets and the last port", async () => {
  ...
  try {
    await Effect.gen(function*() {
      ...
      yield* listener.protocol().pipe(Scope.provide(mcp))
      const first = ticket()
      expect(first).not.toBeNull()
      yield* listener.websocket().pipe(Scope.provide(ws))
      yield* listener.assets().pipe(Scope.provide(app))
      ...
      const otherSocket = yield* Scope.fork(parent)
      yield* listener.websocket().pipe(Scope.provide(otherSocket))
      yield* Scope.close(ws, Exit.void)
      expect(yield* status("/olai/who")).toBe(204)
      ws = yield* Scope.fork(parent)
      yield* listener.websocket().pipe(Scope.provide(ws))
      yield* Scope.close(otherSocket, Exit.void)
      expect(yield* status("/olai/who")).toBe(204)
