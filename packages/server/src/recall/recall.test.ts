 ...
 *
 * That is the kolu-ci-1 rule made structural: a unit lane must not spawn a
 * model server, so everything semantic is proved against a deterministic fake
 * whose only notion of meaning is a synonym table. Two words that share a
 * concept share a dimension; cosine does the rest, exactly as it will for a
 * real model's vectors. The REAL embedder — the one the nix closure carries —
 * is exercised end to end by the e2e suite against the packaged binary, which
 * is the only place it exists.
 *
 * The two PINS the design ships under (docs/brainstorming/semantic-recall.md):
 *
...
test("PIN (degradation): `OLAI_RECALL=off` is the same `null`, embedder or no embedder", async () => {
  // The reader's own off switch (docs/running.md), and it must reach the same
