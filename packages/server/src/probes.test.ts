...
const asks = (name: string) => ({
  name,
  apply: (own: Context) => {
    own.on("chat/session-start", (start, next) => {
      ...
      start.asking.push({ name, ask: async (): Promise<Probed> => ({ server: null, missing: null }) })
      return next()
    })
  },
})
...
test("two plugins that registered back to front are asked in the bundle's order", async () => {
  ...
  await ctx.plugin(asks(SECOND))
  await ctx.plugin(asks(FIRST))
  expect((await askingAt(ctx)).map((one) => one.name)).toEqual([FIRST, SECOND])
})
...
test("a plugin the bundle never named goes last, and the rows keep their order", async () => {
  const ctx = new Context()
  await ctx.plugin(asks("stranger"))
  await ctx.plugin(asks(SECOND))
  await ctx.plugin(asks(FIRST))
  expect((await askingAt(ctx)).map((one) => one.name)).toEqual([FIRST, SECOND, "stranger"])
})
...
test("a plugin whose fiber is gone is not asked, however the bundle lists it", async () => {
  const ctx = new Context()
  const leaving = await ctx.plugin(asks(FIRST))
  await ctx.plugin(asks(SECOND))
  await leaving.dispose()
  expect((await askingAt(ctx)).map((one) => one.name)).toEqual([SECOND])
})
