...
describe("what changed", () => {
  test("a node nobody had before is created", () => {
    const changes = changesOf(at("a.org"), at("a.org", node({ id: "x" })))
    expect(changes).toEqual([
      { file: "a.org", id: "x", title: "x", fields: [], sort: "created" },
    ])
  })
  ...
  test("a mark that appeared and one that was taken off are opposites", () => {
    ...
    expect(changesOf(at("a.org", open), at("a.org", done))[0]).toMatchObject({
      fields: ["done"],
      sort: "done",
    })
    expect(changesOf(at("a.org", done), at("a.org", open))[0]).toMatchObject({
      sort: "undone",
    })
  })

  test("a node that changed file is archived, and reads under the file it is in now", () => {
    const before = at("a.org", node({ id: "x", title: "install them" }))
    const after = at("_olai/Trash.org", node({ id: "x", title: "install them" }))
  ...
  test("a node that is nowhere on the new side is gone", () => {
    const changes = changesOf(at("a.org", node({ id: "x" })), at("a.org"))
  ...
  test("an unchanged node is not a change", () => {
    ...
    expect(changesOf(at("a.org", same), at("a.org", same))).toEqual([])
  })
  ...
  test("a property that changed, appeared or went is reported once", () => {
    ...
    for (const [before, after] of [[none, one], [one, other], [one, two], [one, none]]) {
      expect(changesOf(at("a.org", before as string), at("a.org", after as string))[0])
  ...
  test("a re-stamp is not a change, whatever else the node carries", () => {
    ...
    for (const record of [bare, props]) {
      expect(changesOf(
        at("a.org", node({ ...record, changed: "2026-08-15T09:00:00-04:00" })),
        at("a.org", node({ ...record, changed: "2026-08-15T21:00:00-04:00" })),
      )).toEqual([])
    }
  })
  ...
  test("a field that changed is not joined by a phantom `custom`", () => {
    ...
    expect(changesOf(at("a.org", before), at("a.org", after))[0]?.fields).toEqual(["title"])
  })
  ...
  test("every field that differs is reported, and the sort is the biggest of them", () => {
    ...
    const change = changesOf(at("a.org", before), at("a.org", after))[0]
  ...
  test("a mirror answers by the id it was named by", () => {
    ...
    const before = at("a.org", mirror("a0"), `{"id":"x","ord":"a1","title":"x"}`)
    const after = at("a.org", mirror("a2"), `{"id":"x","ord":"a1","title":"x"}`)
...
describe("the biggest one", () => {
  test("is by the fixed priority, and the first read among equals", () => {
    const changes = changesOf(
      at("a.org", node({ id: "keep" }), node({ id: "mark", ord: "a1" })),
      at(
        "a.org",
