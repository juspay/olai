 ...
 * walks it replaced.
 *
 * IT IS A LEG, NOT A CLAIM (`just bench`), and this one is the whole of what
 * `perf-dates-index` was filed on. That node named three full-vault walks — the
 * calendar's dots, a day page, the agenda's three stretches — and said the fix
 * shape the house already owns is an index on `Derived` maintained by the
 * patcher. A number nobody can re-run is a number nobody can check, so the
 * before and the after are arms of one run rather than two paragraphs, and the
 * node's three bullets are three PAIRS here rather than one blended figure.
 *
 ...
 * directory.
 *
 * SEVEN ARMS over one generated vault (`./fixtures.testlib.ts`'s `vaultOf`, the
 * SAME corpus `./patch.bench.ts` and `./vocabulary.bench.ts` run on, so this
 * leg's numbers and theirs are about one directory): three readings, each asked
 * of the index and of the walk, and the FOLD.
 *
 ...
 *
 * THE FOLD is the half a ratio never shows: `dateInto` over every record, which
 * is the work `derive` gained. It answers a different thing from the six above,
 * so it is timed beside them and compared with none of them — and it is the
 * number that says what the trade IS, since it is paid once per rebuild where a
 * walk was paid once per read.
 *
 * EACH PAIR MUST ANSWER THE SAME VALUE, asserted before anything is timed. It
...
import { type Agenda, agendaOf, owedOf } from "./agenda.ts"
...
/** What the index costs the fold: `dateInto` over every record, which is what
 *  `derive` gained. The map is built and thrown away, exactly as the index is
 *  built and kept. */
...
const READINGS = [
  ...
  {
    what: "the agenda, and the count beside it",
    index: (derived: Derived) => agendaSaid(agendaOf(derived, TODAY)),
    walk: (derived: Derived) => agendaSaid(walkedAgenda(derived, TODAY)),
  },
] as const

// THE SAME ANSWER, per pair, before anything is timed — see the header.
for (const reading of READINGS) {
  const found = reading.index(view)
  const walked = reading.walk(view)
  if (found !== walked) {
    throw new Error(
      `the index and the walk disagree about ${reading.what}, so neither number` +
        ` means anything:\n  index: ${found}\n  walk:  ${walked}`,
    )
  }
}
...
for (const reading of READINGS) {
  ...
  console.log(`  → ${(median(walk) / median(index)).toFixed(1)}×\n`)
}
