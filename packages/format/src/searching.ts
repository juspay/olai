...
export const SearchRequest = Schema.Struct({
  text: Schema.String.annotate({
    description:
      ...
      "- `is:blocked` — the node is WAITING: an `after` edge of its own points at a task that is not finished. Derived, and the same derivation the app draws a blocked row with — a node with no mark is not blocked (a bullet is not work), a target with no mark blocks nothing, and archived work is out of it at both ends. `-is:blocked` is what can be started.\n" +
