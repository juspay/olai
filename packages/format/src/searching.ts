...
export const SearchRequest = Schema.Struct({
  text: Schema.String.annotate({
    description:
      ...
      "- `\"kitchen remodel\"` — a quoted PHRASE is ONE substring where two words are two, looked for in the same four fields. A quoted token is also TEXT rather than an operator, which is the only way to search for the spelling of one: `\"is:done\"` finds the note that says so. Elsewhere in a token a quote only suspends the space, which is how a value that has one is written: `prop:stage=\"in review\"`. A quote nothing closes is refused rather than closed on your behalf.\n" +
      "- `walnut OR birch` — either one. `OR` joins the tokens on either side of it and binds TIGHTER than the space between two tokens, which is the implicit AND: `#home kitchen OR bathroom` is `#home` AND one of the other two, never `(#home and kitchen)` or every bathroom in the directory. A chain is one group, and clauses can be alternatives (`is:todo OR is:doing`). IN CAPITALS — the one token that is not case-folded, because lower-case `or` is an ordinary word to find, and `\"OR\"` is that word in capitals. There are no parentheses: a group is negated by negating each side (`-a -b`, which is \"neither\").\n" +
