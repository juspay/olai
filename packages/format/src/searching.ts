...
export const SearchRequest = Schema.Struct({
  text: Schema.String.annotate({
    description:
      ...
      "- `prop:records=190..200` / `prop:dispatched=2026-08-20..` — a SPAN, on a key the vault DECLARES `int` or `date` in `_olai/Properties.org`. The values are `date:`'s own, whole: days, months, years, the relative words, durations, and any of them at either end of a range. An `int` compares as a NUMBER, which is what declaring it bought — `prop:records=9..200` does not answer `1000` the way a string comparison would. A range on any other key is REFUSED, naming what the key is: comparing text as if it were a date is the lie types exist to prevent. Equality on a `ref` is unchanged, and a ref's value is the variant's ID (`prop:merge=auto`), never its title.\n" +
