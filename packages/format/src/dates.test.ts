...
test("two records claiming one id are two rows, not one", () => {
  const duplicated = derive(
    nodesOfFiles({
      "a.jsonl": `{"id":"dup","ord":"a0","title":"one","props":{"status":"done","since":"2026-08-11T09:00:00-04:00"}}`,
      "b.jsonl": `{"id":"dup","ord":"a0","title":"the other","props":{"status":"done","since":"2026-08-11T10:00:00-04:00"}}`,
    }),
  )
...
test("a mirror of a dated node does not put it on the day twice", () => {
  const mirrored = derive(
    nodesOfFiles({
      "work.jsonl": `{"id":"posts","ord":"a0","title":"dig","props":{"date":"2026-08-05"}}`,
