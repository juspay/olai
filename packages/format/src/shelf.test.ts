...
const setWith = (pins: string, garden = GARDEN) =>
  derive(recordsOf(setOf({ "Pins.org": pins, "garden.org": garden })))

// ── the rows ───────────────────────────────────────────────────────────

test("the shelf is the top level of Pins.org, in ord order, mirrors left out", () => {
...
test("a mirror is not a pin — a placement is not a door", () => {
  const withMirror = derive(
    recordsOf(setOf({
      "Pins.org": `{"id":"p-mirror","ord":"a0","mirror":"herbs"}`,
      "garden.org": GARDEN,
    })),
  )
...
test("the shelf is found by NAME, wherever the directory keeps it", () => {
  const nested = derive(
    recordsOf(setOf({ "notes/pins.org": `{"id":"p","ord":"a0","title":"/agenda"}` })),
  )
  expect(shelfOf(nested).map((row) => row.id)).toEqual(["p"])
})

test("a directory with no shelf has none", () => {
  expect(shelfOf(derive(recordsOf(setOf({ "garden.org": GARDEN }))))).toEqual(NO_PINS)
})
...
test("the QUALIFIED node spelling resolves to the same node", () => {
  // `garden.org#herbs` is what somebody writes when they know where the node
  ...
  const set = setWith(`{"id":"p","ord":"a0","title":"/garden.org#herbs"}`)
