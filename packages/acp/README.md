...
| `usage.ts` | the protocol's `usage_update`, read as data: how full the conversation's context is, and how big the window is. Here rather than in one of `@olai/chat`'s legs because it is ACP's own update kind — any agent may send one — and the cost it also carries is deliberately not read |
...
What may cross the boundary is machine-checked rather than agreed by comment: `src/manifest.test.ts` enumerates the package's imports (only `effect` and the SDK's types), its closed export list, where the SDK may be imported at all (here and `@olai/chat`), and who may import this package (`@olai/surface` for the wire half, `@olai/chat` for the projections).
