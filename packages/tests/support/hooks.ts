 ...
 * harness explicitly adds ws, web-app, mcp, ui-renderer and layout: its tab and agent tool carrier.
...
Before(
  ...
  async function (this: OlaiWorld, scenario) {
    ...
    this.pluginPin = scenario.pickle.tags.flatMap((tag) => {
      ...
      return asked === null ? [] : [asked[1] === "none" ? "" : [...new Set([...asked[1]!.split(","), "ws", "web-app", "mcp", "ui-renderer", "layout"])].join(",")];
    })[0];
