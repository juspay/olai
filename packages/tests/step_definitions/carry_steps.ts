...
import { CHAT_INPUT, CHAT_ENTRY, CHAT_DROP, POLL_TIMEOUT } from "../support/world.ts";
...
When("I quote the last {string} row into the conversation", async function(this: OlaiWorld, kind: string) {
  const entry = this.chat(`${CHAT_ENTRY}[data-kind="${kind}"]`).last();
  const grip = entry.locator("..").locator('[data-testid="chat-grip"]').first();
  await entry.hover();
...
When("I drop sidebar file {string} into the conversation", async function(this: OlaiWorld, path: string) {
  const source = await this.box(this.page.locator(`nav a[title="${path}"]`).first(), "file row");
