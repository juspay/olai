...
This suite is 64 features, 781 scenarios (773 Gherkin `Scenario`/`Scenario Outline` entries; four outlines expand to twelve examples), 558 `@scratch:`. 26 features carry `@share-scratch` (375 sharing scenarios, 13 `@own-scratch` inside them). A 2026-08-19 audit cut ~160 that did not earn the browser; the grammar, the destination refusals, the trash wording, the install fetch and the rest of that list live in the unit suites now. Four browser-only claims the first cut left unpinned (anchor jump, sidebar inner scroll, same-page never-inside-itself, a preview's height after a late picture) are back.
