...
This suite is 79 features, 1028 scenarios (1018 Gherkin `Scenario`/`Scenario Outline` entries; five outlines expand to fifteen examples), 693 of them running with a `@scratch:` copy — counted from the features as they stand, scenario-level tags and feature-level inheritance both. 30 features carry `@share-scratch` (433 sharing scenarios, 28 `@own-scratch` inside them). A 2026-08-19 audit cut ~160 that did not earn the browser; the grammar, the destination refusals, the trash wording, the install fetch and the rest of that list live in the unit suites now. Four browser-only claims the first cut left unpinned (anchor jump, sidebar inner scroll, same-page never-inside-itself, a preview's height after a late picture) are back.
