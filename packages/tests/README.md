...
This suite is 79 features, 1031 scenarios (1021 Gherkin `Scenario`/`Scenario Outline` entries; five outlines expand to fifteen examples). The census reads TAGS the Gherkin way: a tag line is a line carrying nothing but tags — a mention mid-comment or mid-prose (this paragraph's own, for instance) is no tag — the tag-block above `Feature:` inherits onto every scenario of the file, the block above a scenario is its own, and the counts speak in ENTRIES: an outline's three examples count once — the two TAGGED outlines (`daily_notes.feature`'s `@scratch:journal`, `documents.feature`'s own) expand only on the runs side of the ledger, where the same claim would answer 734: the answer this frame gives is entries. 730 of the 1021 entries run under a scratch copy. 31 features carry `@share-scratch`: their 463 entries split 440 sharing the feature's one copy and 23 with an `@own-scratch` of their own inside. A 2026-08-19 audit cut ~160 that did not earn the browser; the grammar, the destination refusals, the trash wording, the install fetch and the rest of that list live in the unit suites now. Four browser-only claims the first cut left unpinned (anchor jump, sidebar inner scroll, same-page never-inside-itself, a preview's height after a late picture) are back.
