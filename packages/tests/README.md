...
```bash
odu run                       # full CI; E2E borrows up to six execution slots
just e2e                      # the whole E2E suite on this machine
just e2e-fast-remote          # only E2E through this tree's pinned Odu
```
...
`odu run` is the one full-CI command: it expands `just check`, fans the platform legs out to the configured SSH/Nix machines, posts the stable logical contexts to GitHub, and opportunistically borrows up to six free slots for `e2e`. Six is a ceiling, not a fleet reservation. If only three slots are available, Odu tells Cucumber to run three complete, non-overlapping slices; a one-slot run is the whole suite. Every leased lane immediately dispatches its own `install` and `nix` prerequisites in parallel; Odu reports the burst copies as nodes such as `e2e[2-of-3]::install`, so a slow shard start is visible instead of being charged to E2E. The logical E2E duration is the slowest slice's execution time. The ordinary platform lease remains held for the CI run, while each extra E2E lease is released as its slice finishes. `just e2e-fast-remote` is only a thin pinned-Odu spelling of `odu run e2e --platform x86_64-linux`; it contains no SSH scheduler of its own. It has a 10-minute wall-clock backstop (`ODU_E2E_REMOTE_TIMEOUT` to override) and interrupts Odu cleanly before forcing termination.
...
Lane evidence is [saatchi](https://github.com/juspay/saatchi): a throwaway `.saatchi/evidence.ts` section, photographed against the app the `serve` recipe starts. The adapter and fixtures live in `.saatchi/` at the repo root. Shots land in `.saatchi/shots/` and go on the PR body, never in git. `just e2e` never runs this. The promises live in the features; this is what a person looks at.
...
`reads.ts` / `reads.sh` are a driver here and are not part of the suite either. `wire.ts` counts bytes because what it is showing is a COST. What a tool surface has to show is neither a look nor a cost: the claim is "this question is one call now", and the only honest exhibit is the call and the answer printed beside it. So this connects to `/mcp` exactly as a `.mcp.json` client does, runs a scripted read session, and prints every request and every answer — the refusals included, since a refusal is an answer here and its `kind` travelling as data is half of what the surface promises.
...
fixture leaves keep their values. An exact `@rows:` selection retains default
reader rows. File `on` on reader owners is ignored; scenarios needing their
absence stop them through the panel.
...
**`@zone:<IANA name>`** (`@zone:America/New_York`) runs the scenario's browser in that time zone. Without it the browser keeps the zone of the machine running the suite, which is fine for anything that asks the local day and wrong for a scenario that asserts the offset a write is stamped with — a CI box in UTC and a laptop in New York would expect two different files. Name a zone that moves its clocks, so an offset taken from today rather than from the moment shows up as a failure.
...
| `[data-testid="stale-banner"]` | shown OVER pages that are LIVE: which files of the directory are broken. It keeps the name it had when it meant "showing the last good version", which is now only what it says for a directory that could not be READ |
| `[data-testid="broken-file-line"][data-file][data-state]` | ONE broken file's line inside that summary — its path, its state (`unreadable`/`unparsed`/`invalid`) and a row COUNT. Never the rows: it is drawn over somebody else's page |
| `[data-testid="broken-file-link"][data-file]` | …and the door on that line, to the broken file's own page |
