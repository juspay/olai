...
This suite is 62 features, 745 scenarios (741 Gherkin `Scenario`/`Scenario Outline` entries; two outlines expand to six examples). A 2026-08-19 audit cut ~160 that did not earn the browser; the grammar, the destination refusals, the trash wording, the install fetch and the rest of that list live in the unit suites now. Four browser-only claims the first cut left unpinned (anchor jump, sidebar inner scroll, same-page never-inside-itself, late-picture rungs) are back.
...
│   ├── scratch.ts           # @share-scratch / @own-scratch, the hash-diff, the collision
...
**`@share-scratch`** is how a feature whose scenarios write DISJOINT files shares one copy and one server, per worker. Put it at the top of the feature; every `@scratch:` scenario in that file on this worker reuses the same process. Sharing never crosses workers: four workers running the same feature are still four olai, for the same lock as the corpus servers. After each sharing scenario the harness hashes the scratch tree; if this one changed a path an earlier one on the same worker already changed, the run fails naming both scenarios and the file — never a silent flake. **`@own-scratch`** on a scenario inside a sharing feature keeps a private copy (overlapping writes, an exact listing of the vault, a server restart). It is refused without `@share-scratch`, because a tag that does nothing is how an author thinks they opted out and did not.

Observation, not a `@writes:` declaration: a declaration can be forgotten, and a forgotten declaration is the flake. The hash-diff sees UI edits, `writeServed`, the agent, Trash minting. It does not see a scenario that only *reads* a file another one wrote, so a feature whose early scenarios depend on the original corpus and whose later ones mutate it is not a candidate — convert only where the writes are disjoint *and* the readers do not care about those files. Chat features cannot share even when files look disjoint: conversation state lives in the server and in XDG.
...
The names are not written down twice. `support/world.ts` imports the client's own `TESTID` record and its `selector()` helper from `packages/web/src/client/testids.ts` (the first reason `@olai/web` is a dependency of this package), and builds every constant from it. The same rule covers the handful of other constants a step would otherwise re-spell — the day arithmetic in `clock.ts`, the theme table's attribute, storage key and default — but never behaviour: these tests drive the client through a browser, not through its modules. A renamed testid is therefore a type error at `bun run typecheck`, not a thirty-second timeout in a scenario that no longer says why it failed. `#root` stays spelled out locally: it is `index.html`'s mount point, which the client does not own.
...
5. If it needs to CHANGE a file, tag it `@scratch:<corpus>` and write through `world.writeServed`. If every scratch scenario in the feature writes a *different* file, put `@share-scratch` at the top of the feature rather than paying a process spawn per scenario; tag a collider `@own-scratch`. The assertions that follow such a write usually need to wait for something to change or disappear, which a Playwright selector cannot state — `world.waitUntil` is what those steps are built on.
