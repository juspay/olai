...
| `[data-testid="blocked"]` | a node waiting on unfinished work — the pill on a row, the named blockers on a zoomed page — and NOT PRESENT when nothing is in its way |
| `[data-testid="see-refs"]` | a node's free cross-references (`see`) |
| `[data-testid="node-ref"]` | one link from a node to another node inside either of those rows; the target id rides `data-ref` on a span inside it, and the blocked pill carries that span too |
