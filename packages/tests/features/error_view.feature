Feature: The error view
  ...
  Scenario: A line that is not JSON is named by file and line
    ...
    And the error group for "pantry.olai" is shown
    And the error group for "pantry.olai" shows an error at "pantry.olai:3"

  @corpus:tangled
  Scenario: Every error is listed, and every single-file error sits under its file
    When I open the app
    Then the error view is shown
    And an error is listed at "attic.olai:3"
    And an error is listed at "cellar.olai:2"
    And an error is listed at "cellar.olai:3"
    And an error is listed at "cellar.olai:4"
    And the error group for "attic.olai" shows an error at "attic.olai:3"
    And the error group for "cellar.olai" shows an error at "cellar.olai:4"

  @corpus:tangled
  Scenario: Errors implicating two files get their own section
    # cellar.olai:2 names a parent that lives in attic.olai; cellar.olai:3
    # re-uses an id attic.olai:2 claimed first. Neither file is "the" broken one.
    When I open the app
  ...
  Scenario: A single-file error stays out of the cross-file section
    # cellar.olai:4 points at `nowhere` — nothing declares it, so no second
    # file is implicated and the error belongs to cellar.olai alone.
    When I open the app
