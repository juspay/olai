...
Feature: The connection is visible, and a replaced server is recoverable
  ...
  The state that must never look healthy is the last one here. When a server is
  restarted, the tab reconnects presenting the process id it was given by the
  server that is gone; the new one does not recognise it and closes the socket
  at the handshake, which retires the wire for good — no reconnect will ever
  come. There is nothing to wait for, so the page stops pretending and offers
  the only thing that works: a reload.
  ...
  Scenario: A restarted server retires the tab, and reloading recovers it
    ...
    # The gate the client's `pid` echo finally feeds: the server closed this
    # tab rather than serving it. Asserted from the server's own log, because a
    # page cannot see the difference between the socket it was refused and one
    # that simply has not opened yet.
