...
| Chat drafts and context | Text and @ completion, node chips, refusal recovery, drawer/session changes, stale/deleted context, reload boundaries | `chat_at_completion`, `chat_at_nodes`, `node_context`, `chat_plugin_lifecycle` | Remount draft loss and stale-tab sends fixed. Three new scenarios cover delayed sends with attachment/text recovery, a newer draft typed before refusal and the palette send path. Remaining context/retry edges open |
