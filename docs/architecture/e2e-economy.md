...
| `tabs/src/store.browsertest.ts`: the lane following the strip, the order of forget, record and switch, `open` without a lane, ids, the restored front's address and name | The strip registering itself through a real layout shell, and the router behind it being a real history (`strip.feature`, `phone.feature`, `withdrawal.feature`) |
