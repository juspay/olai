...
**Which of them are ids is asked of the server**, once per message — the browser used to answer it out of its own copy of the whole directory, and that copy is what it is giving up ([brainstorming/vault-in-browser.md](https://github.com/juspay/oss.olai/blob/master/olai/brainstorming/vault-in-browser.md)). Two things follow, and both are visible:
