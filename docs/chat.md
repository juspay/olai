...
- **Codex uses native child sessions.** Each announced child gets an agent card; its tools and nested agents belong behind that card's work door, its prose stays in its fold, and its questions stay visible and attributed in the main conversation. Codex background terminals use the existing task rows and running strip, including active terminals announced on session load. Their lifecycle is independent of the main turn. Individual task stopping is not exposed by the panel. See [Codex](plugins/codex.md) for the negotiated adapter extensions.
