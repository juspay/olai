...
- **Claude Code and Codex can be INTERRUPTED.** Both advertise `_session/steering`, so the control is offered from a positive handshake fact rather than from the engine's name. Opencode has no such method, and neither do pi or omp; those rows simply draw no interrupt gesture ([below](#talking-while-it-works)). Pi instead queues a mid-turn message and announces its position. Codex advertises steering without ordinary prompt queueing, so normal busy sends use steering too. **omp is the one where a busy send is not a queue at all: it REPLACES the turn.** A message sent while omp works cancels the turn in flight and runs what you just sent, so the first row ends *cancelled* — the same row a person gets when they press stop — and the second is answered. That is steering-by-default on omp's own wire, and it advertises no ACP way to interrupt precisely because a send already is one; the composer therefore promises nothing about a mid-turn message, and [omp](plugins/omp.md) says what actually happens instead.
- **opencode, pi and omp subagent activity is drawn flat** — every call in one column — rather than in lanes ([below](#when-the-agent-sends-other-agents)).
...
- **a tool call's name comes from wherever that agent says it.** Claude Code says it in a field of its own; opencode, pi and omp say it at the head of the call id (`bash:0`, `edit:1`, `write:0`). Codex's title is display text while its id is opaque, so this plugin names no tool for the approval boundary. In every case a tool olai cannot positively name is one you are asked about rather than one quietly allowed.
...
**Press the model name to choose another model.** The menu uses the options supplied by the current agent and is available while the conversation is idle. It opens with a filter box first: type to narrow the list, use the arrow keys to walk it, and Enter to take the row under the cursor. The box matches the agent's own id as well as the displayed name, so an agent whose choices are spelled `provider/id` can be narrowed to a provider by its name. Olai sends the selection through ACP configuration, so this works with Codex even though its adapter has no `/model` command. The agent line changes when the agent confirms the choice; a refusal leaves the current model in place and shows the reason. The choice is remembered for the restored conversation. Agents that supply no model options keep a plain model label.
...
**If an interruption answers but stays busy, cancel releases it.** Older Claude bundles could also hang after arming a **watch**, even when nothing had queued. That sequence no longer reproduced with the current bundle; interruption while a watch is still live remains unverified. The button is still offered in a conversation that has never queued. If your words have been answered but the turn stays on *working…*, **cancel** ends it without losing the conversation. The pin's checks and their limits are recorded in `packages/plugins/claude/acp/patches/README.md`.
...
**The node half is the search you already know** — literally the same one, asked of the server, so `@cab` here selects what `cab` selects in the filter bar, in the `⌘K` palette and in an agent's `search_nodes` — title, id, tag and note, ranked the same way, with a finished node losing ties ([search.md](search.md)). Spaces keep narrowing the list, so `@odu watcher scope` can name the row titled “Odu watcher scope”. Search expressions such as `@is:blocked`, `@#home`, `@date:today`, quoted phrases and `OR` use the same search grammar. The query runs from `@` to the caret; a newline or tab ends it. For a query containing a space, rows start unselected and **Enter sends the message as written**. Use ↑/↓ to select a row and then Enter, or use Tab or a click to complete directly. Typing more query text or moving to a different `@` clears the selection. A live result replacing the selected row is not selected automatically. Completion replaces that span with its handle, preserving the sentence after the caret and returning the caret immediately after the inserted handle and any following punctuation, including when the row is clicked. Acceptance closes that offer so the next words are ordinary prose. Escape closes it without changing any text. To revise a closed offer, move the caret before its `@` and back, or remove the `@` and type it again. Type the spaces as they appear in the title: running the words together (`@oduwa`) does not match “Odu watcher scope”. `@` **names** one node; the palette **searches**.
...
Arrow selection belongs to the open menu and is cleared when that menu is remounted. A dismissed completion stays dismissed with that conversation’s draft when its fold is closed, another node’s conversation is opened, or the plugin runtime rebuilds. Returning to the draft preserves Enter as Send. A different token or a new message can offer completions again.

While the list is up, ↑/↓ walk it — through both blocks, one cursor — Tab takes a row and Escape closes it. Enter takes the selected row; a query with spaces has no selection until an arrow key chooses one, so Enter otherwise sends. A changed query resets the cursor; a repeated caret notification for the same query preserves the row the arrows reached. A click does the same for a hand already on the mouse. It is the same box the `/` commands use, because it is the same gesture.

**When Enter is completing, it takes a selected row of the list you are looking at.** The node half is asked of the server, so it settles for a fifth of a second before it asks and those rows hold still until the next ones land. Enter on a selected row inside that gap writes nothing rather than putting the word before last's node into your sentence and arming it; the rows catch up a moment later, and the same key takes the one you meant ([editing.md](editing.md) says it where the other lists in this app say it). The FILE rows are matched in your own tab, so they are never behind anything: `@cab` and Enter writes a path at once, as it always has. And a click is never held back at either half — your hand is on the row you can see.
...
- **What the wire carries** is exactly that, and only because olai patches the adapter it ships with (`packages/plugins/claude/acp/patches/README.md`). As released, that adapter completes such a call at the moment it launches — the acknowledgement read as the result — so an armed watch and a finished one were the same row and there was nothing to draw a clock or a death from. The Claude leg does not currently negotiate AIR; the adapter measurement and switch constraints live in the patch README's “What an AIR client gets from #1017 (2026-09-23)” section. The patch is [PR #941](https://github.com/agentclientprotocol/claude-agent-acp/pull/941)'s approach on [issue #865](https://github.com/agentclientprotocol/claude-agent-acp/issues/865), extended from async agents to every task the harness registers.
...
Nothing above is guessed from a tool's name. Whether a call sent an agent out, and which agent a call was made inside, are two facts olai reads off the frames — and an agent that carries neither (opencode does not, nor does omp) is drawn exactly as it always was: one column, every call in it, no strip entries, no doors and no shelf. That is the direction this is safe to be wrong in, and it is the direction it is wrong in.
...
The strip has one row per plugin that contributes a wake control. Kolu and odu own their file pickers; mail owns a **wake on new mail · off|on** switch. Chat remembers each choice for this conversation and draws the count of events waiting behind its current turn. See [mail](plugins/mail.md#waking-an-agent-when-mail-arrives) for its cadence and a sample triage charter.

**A plugin can put a message into this conversation, and you decide which one it may put it into.** Under the roster and the strip of what is running is a third line — one per plugin that has something to watch — saying what the wake would be about and the choice you made:
