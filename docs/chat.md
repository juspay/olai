...
**The node half is the search you already know** — literally the same one, asked of the server, so `@cab` here selects what `cab` selects in the filter bar, in the `⌘K` palette and in an agent's `search_nodes` — title, id, tag and note, ranked the same way, with a finished node losing ties ([search.md](search.md)). Spaces keep narrowing the list, so `@odu watcher scope` can name the row titled “Odu watcher scope”. Search expressions such as `@is:blocked`, `@#home`, `@date:today`, quoted phrases and `OR` use the same search grammar. The query runs from `@` to the caret; a newline or tab ends it. Enter, Tab or a click takes the row and replaces that span with its handle, preserving the sentence after the caret. Acceptance closes that offer so the next words are ordinary prose. Escape closes it without changing any text. To revise a closed offer, move the caret before its `@` and back, or remove the `@` and type it again. `@` **names** one node; the palette **searches**.
