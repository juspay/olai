...
**Once you have sent a message that had to wait, this conversation stops offering it.** The button goes and Alt+Enter becomes an ordinary send. That is a guard around a bug in the agent adapter olai pins, not a decision about what is useful: interrupting a turn in a conversation that has ever queued leaves that turn never finishing — the words you interrupted with arrive and are answered, but the panel stays on *working…* until you press **cancel**, which does end it and loses nothing. Rather than hand you a button that does that, the panel takes it away for the rest of the conversation. **A new conversation gets it back** (`+ new`, or opening a stored one), because the problem is per conversation. It goes away for good when the pinned adapter is fixed — the September 2026 pin bump moved the adapter four releases and the bug is still there, measured, so it is not a thing to expect from the next one either.

**The guard does not catch every way in, and this is the one it misses.** A conversation in which the agent has armed a **watch** — the `Monitor` tool, the thing whose clock the strip under the header draws — hangs the same way when you interrupt it, and because nothing has queued there the button is still on offer. (A shell sent to the background does not do it; a watch does. Both were measured.) It is the same recoverable shape: what you interrupted with arrives and is answered, and **cancel** ends the turn and loses nothing. It is written down here rather than guarded because widening the guard is a change to what the panel offers, and that is a decision rather than a bump.
