...
**A message typed while a conversation is opening waits for it.** Opening one takes real time — the agent replays the whole conversation before it answers — and the box is not locked while it does, so the next thing you type lands in the gap. It goes into the conversation being opened, once it is: nothing is refused and nothing is lost. Before this it started a second open of its own, against the first, and the message died with it.
