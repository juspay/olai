...
When connected, [mail](plugins/mail.md) provides inbox, search, thread and attachment reads, plus archive, Trash, restore, labels and read marks. These nine tools live on olai's own MCP server and appear under **olai ✓**, with a one-line result in each tool row.
