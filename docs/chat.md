...
Arrow selection belongs to the open menu and is cleared when that menu is remounted. A dismissed completion stays dismissed with that conversation’s draft when the panel is closed, another node chat is selected, or the plugin runtime rebuilds. Returning to the draft preserves Enter as Send. A different token or a new message can offer completions again.

While the list is up, ↑/↓ walk it — through both blocks, one cursor — Tab takes a row and Escape closes it. Enter takes the selected row; a query with spaces has no selection until an arrow key chooses one, so Enter otherwise sends. A click does the same for a hand already on the mouse. It is the same box the `/` commands use, because it is the same gesture.
