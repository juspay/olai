...
* a day the journal **has a node for** is a link to that node's page (`/n/<key>`, minted like every other), in full ink, with a dot under the number;
* a day it has nothing for is a dim number and nothing else — **inert**, because there is no page to open and the view writes nothing;
* **today** wears a green ring, whichever of the two it is;
* the day you are **reading** is filled — ink ground, paper number, and `aria-current="page"`. Today and open is the fill inside the ring.

The month header zooms to the node this month's days hang under — the whole journal, read as an outline — and falls back to the root's first node in a month nothing has been written in yet. It is today's month and only today's: the sidebar is a live region that re-fetches whatever page drew it, so a month you could page through would have to ride in every page's address or in client state the morph has to keep. Pure view over the snapshot — no stored state, no writes, and a directory with no `Daily.rkt` sees no change at all.
