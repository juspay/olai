...
**The month.** One root is not drawn as a name and a tree: `Daily.rkt` — recognised by that basename, the way `Archive.rkt` is — becomes a mini calendar of the month around today, standing exactly where the file's entry was. A day the journal has a node for is a link to that node's page (`/n/<key>`, minted like every other); a day it has nothing for is a number and nothing else, inert, because there is no page to open and the view writes nothing. Today is marked whichever of the two it is. The month header zooms to the node this month's days hang under — the whole journal, read as an outline — and falls back to the root's first node in a month nothing has been written in yet. It is today's month and only today's: the sidebar is a live region that re-fetches whatever page drew it, so a month you could page through would have to ride in every page's address or in client state the morph has to keep. Pure view over the snapshot — no stored state, no writes, and a directory with no `Daily.rkt` sees no change at all.
...
Nothing is indexed in the browser and nothing is sent as JSON. The box carries the same partial-fetch attributes a link does (`live/README.md`'s `live-query`), debounced 200ms: it re-fetches `/search?q=…` and morphs the results region — and only that region — into place. The palette itself sits outside every live region, like the chat panel, so a file saved while you are typing cannot take what you typed with it; the results ARE a region, so a rename lands in an open palette instead of leaving it pointing at a node that has moved. With no JS at all, the box is a `GET` form and `/search?q=…` is the page it submits to.
