...
**ONE argument, and it is a place, not a file set.** A directory, a single `.rkt`, or nothing at all (which means `$PWD`). Two or more paths is a **usage error** — exit 1, `olai: serve takes one directory or one outline, not 3 (check / tree / agenda take a list)` on stderr — never a set quietly assembled out of them. The read commands above are where a list you typed belongs; `serve` is a server, and what it is pointed at is where it lives: the agent works in that directory, `/media/` reaches into it and nowhere else, and its outlines are re-asked as they change.
