...
**The `•••` menu has the two verbs**: *Link to a node…* and *Wait for a node…*. Each opens a panel under the row holding what the node says now, with an `×` on each of them, and a box that searches the whole directory — the server's own search, the same one the move picker and `((` use, so what this finds and what an agent's `search_nodes` finds cannot drift. Walk the hits with the arrows, take one with **Enter**, leave with **Escape** or **Done**. Choosing one sends the same `set_see` / `set_after` an agent sends, and ⌘Z takes it back.
...
**A pinned page keeps its query, and that is how a SAVED SEARCH is a pin.** `/agenda` filtered to `is:todo` is pinned as that whole address, drawn with the query beside its name, and clicking it lands on the agenda WITH the filter in the box. The same sentence reaches the whole directory since `one-search-box`: `/search?q=%23next` is a page like any other, so pinning it puts a cross-vault search on the shelf — one click to every node in the vault carrying that tag, and still a pin rather than a fourth kind of thing ([search.md](search.md)).
