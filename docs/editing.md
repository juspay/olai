...
Nothing has a mode: the title becomes an input in the same place, at the same size, and the row you are in is toned so you can see where the caret went. In a TITLE what you type is the source — `**bold**` and `#tags` as they are written — and the rendering comes back the moment you leave: one line, one string, and the platform's own caret in it. A NOTE is prose, and prose is where markers are most of what is on the screen, so a note is [live-previewed](#the-markdown-editor) instead — bold looks bold while you write it, and the `**` is there whenever the caret is.
...
Prose in this app — a note under a row, a whole document — is written in an editor that DRAWS the markdown while you are in it. `**bold**` is bold, `# a heading` is heading-sized, `- a list` has a bullet where its dash was, `[ ]` is a box you can press, and a `#tag` wears exactly the ink it wears on the row. The markers are hidden while the caret is elsewhere and **come back the moment the caret is inside them** — put it in the word and the `**` is there to edit, move away and it is drawn again. Obsidian and Typora work this way; so does this.

**A note you have opened IS that editor, reading.** It is not a rendering that gets replaced by an editor when you want to type: it is the same surface, with nothing to type into until you click it, and then the caret is simply in it. Nothing is swapped, so nothing moves — and the caret arrives **where you clicked**, which is the thing a swap could never do, since the words you were aiming at belonged to something that was about to be thrown away. Click a note, and the letter you were pointing at is the letter you are now typing beside.

The exceptions are the places no caret can go, and they are the places that keep the rendering: a day page, the agenda, the trash, a node's own page, what the agent writes in the chat, and a row that is open because your *Notes* preference is **Open** rather than because you opened it — a page of a hundred unfolded notes is a hundred editors, and that is a real second and a half you would wait for nothing. Touch such a row and it becomes the surface like any other.

**And the source is still the whole document.** What the editor holds is the markdown the file holds, character for character; the rich text is a rendering laid over it. Nothing parses your text into a model of its own and writes it back — which is the law this app is built on, and not a small distinction. An editor that round-tripped would rewrite bytes you never touched (`_em_` becomes `*em*`, a list reflows, an escape is dropped), and under autosave that churn would land on every pause: phantom diffs in your git history, and conflicts with an agent editing the same file. Because there is no such step, fidelity is not a promise this app has to keep — it is a thing it cannot break.
...
**Autosave, with nothing to press.** Half a second of quiet, or the caret leaving, and what you typed is on disk. Every write says what this editor last saved, so a write that would land on top of someone else's — an agent, vim, another tab — is refused instead, in the ops layer's own words, on a line under what you were writing, with your text exactly where it is. There is no dirty flag anywhere, because there is nothing for you to discharge.

**Vim mode is in prefs**, under *Editing*, and it is off until you say otherwise. On, both markdown editors are vim editors: modes, hjkl, the block cursor. One key changes hands, and it is the one you would expect — **Escape belongs to vim**, so it switches mode rather than closing the editor; leave a note the way you leave any other, by clicking somewhere else, and leave a document with **Done**. Like every preference it belongs to this browser, reaches every tab of it, and is never sent anywhere.

The editor itself arrives on demand: it is a large piece of machinery, an outline of titles needs none of it, and it is fetched the first time you put a caret in prose. Until it lands — and if it never does — what you are typing in is a plain text box holding the same words, writing to the same file by the same rules, with the markers simply visible.
...
A `.md` under the served directory has always had a page; the page can be written now. **Edit**, on its header, turns the rendered body into its SOURCE — [the same editor a note gets](#the-markdown-editor), one size up, holding the file verbatim.

**A document is the one place the editor is not also the reading surface**, and the reason is what a document page has that a note does not: a [contents](#writing-a-document) derived from its headings, `#heading` links that resolve to real places in the page, footnotes, tables, pictures and highlighted code fences. All six are drawn by the renderer and none by the editor, so the page keeps the renderer for reading and hands the surface over when you press Edit. If you are curious why the split falls there: across every note in this project's own outlines there is not one table, image or footnote — that is what documents are for. There is no toolbar and no WYSIWYG, because a document is markdown and markdown is text. A `.html` in the directory has a page too and does not have that control: olai shows one and never writes one, and it is drawn — and runs — sealed in a sandboxed frame whose origin is nobody’s ([format.md](format.md#hypertext)).

The mode is declared, and that half is unchanged: the rendered body is full of links you are entitled to follow, so a click that put a caret in it would delete the reading surface to save a press. **Done** is the way back to reading, and so is Escape.

**Leaving is not what writes it.** A document autosaves, like a note and by the same rule: half a second of quiet, or the caret leaving, and what is in the editor is on disk. There is no Save button, no Cancel and no dirty flag — the debounce is idle-keyed, so a pause mid-sentence is not a write and walking away from the keyboard cannot lose the paragraph.
