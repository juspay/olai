...
**What goes in leaves every other page** (ruled 2026-08-17). A row in the Trash is off its day, out of the calendar's dots and off the agenda even if it was overdue — putting something away is saying you are done looking at it, and the Trash is the one page that shows it. Nothing on disk changes for that: the record keeps its dates and its mark, **Put back** returns it to its days along with its outline, and `is:archived` finds it from any search box in the meantime ([search.md](search.md)).
