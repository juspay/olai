...
**A chip draws what the file holds.** A `ref`-typed value is the variant's id ([format.md](format.md#typed-properties)), and `auto` is what the chip says — nothing resolves it to the variant's title. That is a door this app has not opened yet rather than a display rule: it lands with the same work that turns a repo and a `pr` number into a link.
