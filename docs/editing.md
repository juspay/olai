...
**It is a drawing rule and nothing else.** Those files are in the directory either way: search finds them, an agent's `outlines_map` lists them, the Trash page and the shelf read them, and git records them like anything else. What changes is one list of rows in the directory column.
