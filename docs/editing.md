...
**A key the vault DECLARED is answered by its declaration** rather than by the list above, because a declaration is a stronger warrant than any guess ([format.md](format.md#what-a-declared-value-names)). A `worktree` declared `path` may point at a directory on some other machine, so nothing here serves it and it is not offered as a door; a `brief` opens the file when the directory has one; a `ref` opens the record it names. A `doc` is held to the *same* answer the write gate gives — it is the one kind that promises its value names a served document, so a value the validator refuses is never a live door here. A declared `text` reads exactly as an undeclared key does, which is what keeps the URL in a `pr-url` a link.
...
**Except one that would not parse.** An outline olai could not read still wears the ⚠ every unreadable outline gets — on its row in the group now, at the home the files live in, because that mark is the only place this app says so short of opening the page: a `Pins.olai` with a bad line in it is otherwise an empty shelf and no explanation anywhere.

**The Trash alone stays out of the group** — it is a page rather than a file you edit, and it already has its door beside the group in the same register ([format.md](format.md)).

**The Inbox entry is drawn only when there is an inbox.** A directory that has never captured has none, and minting one is the capture's job, not a door's — so the entry appears the moment your first `⌘K` `+` lands and opens the file that capture went into. It sits **beside Agenda**, and it wears Agenda's own count badge: how many top-level captures still await processing. A todo or a doing counts; a leaf bullet counts; a bare-bullet header counts only if some descendant still in the open part of the branch is unfinished (a todo, a doing, or a bullet leaf). A done row does not, and neither does a finished branch. The chip hides at zero — an empty inbox is a door, not news. A file that holds only placements is the same zero: a mirror is a view of something elsewhere, not a capture, even though the page behind the door still draws the row. An inbox whose captures are all done, or whose only remaining roots are finished branches, is that zero too: processed work is not news on the door. It is an ordinary outline behind that door — type into it like any other — so the entry lights up when you are on it and carries the ⚠ when the file will not parse.
