...
Nothing has a mode: the title becomes an input in the same place, at the same size, and the row you are in is toned so you can see where the caret went. What you type is the SOURCE — `**bold**` and `#tags` as they are written — and the rendering comes back the moment you leave. A note is the same trade one line down. (A document's own page is not: it is [live-previewed](#the-markdown-editor), so bold looks bold while you write it and the `**` is there whenever the caret is.)
...
**A `.md` page IS this editor.** Open a document and what you are reading is a markdown editor mounted read-only: `**bold**` is bold, `# a heading` is heading-sized, `- a list` has a bullet where its dash was, `[ ]` is a box, a `#tag` wears exactly the ink it wears on a row, and every marker is hidden. Click anywhere in it and the caret is there — in that same surface, at the character you were pointing at. The markers **come back the moment the caret is inside them**: put it in the word and the `**` is there to edit, move away and it is drawn again. Obsidian and Typora work this way; so does this.

There is no **Edit** button and no **Done** button, because there is nothing to switch between. Nothing is swapped for anything, so nothing moves — the words you were aiming at are still exactly where they were when the caret arrives, which a rendering that gets replaced by an editor could never manage. Press **Escape**, or look somewhere else, and the caret leaves; what you typed is already on its way to disk.

**A note is not this yet.** A note under a row is the plain text box it has always been — markers visible, the source as you typed it — and making it this editor too is its own piece of work. Everything below is about a document's page.

**The source is still the whole document.** What the editor holds is the markdown the file holds, character for character; the rich text is a rendering laid over it. Nothing parses your text into a model of its own and writes it back — which is the law this app is built on, and not a small distinction. An editor that round-tripped would rewrite bytes you never touched (`_em_` becomes `*em*`, a list reflows, an escape is dropped), and under autosave that churn would land on every pause: phantom diffs in your git history, and conflicts with an agent editing the same file. Because there is no such step, fidelity is not a promise this app has to keep — it is a thing it cannot break. It goes as far as your **line endings**: a file written on Windows is written back in CRLF, because the editor is told what the file's own break is before it draws a character of it.

The type it is set in is the type the rendering uses: one scale for both, so the heading you are writing is the size of the heading you are about to read, in whichever of the fifteen palettes you are in.

**Autosave, with nothing to press.** Half a second of quiet, or the caret leaving, and what you typed is on disk. Every write says what this editor last saved, so a write that would land on top of someone else's — an agent, vim, another tab — is refused instead, in the ops layer's own words, on a line under what you were writing, with your text exactly where it is. There is no dirty flag anywhere, because there is nothing for you to discharge. And a document you are only READING follows the file: rewrite it in vim and the page you left open re-reads itself, exactly as the old rendering did, with nothing to argue about because nothing was being argued over.

**Vim mode is in prefs**, under *Editing*, and it is off until you say otherwise. On, a document is a vim editor: modes, hjkl, the block cursor. One key changes hands, and it is the one you would expect — **Escape belongs to vim**, so it switches mode rather than giving the caret back; leave by clicking somewhere else. Like every preference it belongs to this browser, reaches every tab of it, and is never sent anywhere.

The editor itself arrives on demand: it is a large piece of machinery, an outline of titles needs none of it, and it is fetched the first time you open a document. Until it lands — and if it never does — the page draws its own rendering and a click puts you in a plain text box holding the same words, writing to the same file by the same rules, with the markers simply visible.
...
A `.md` under the served directory has always had a page; the page can be written now, and it is written IN PLACE — the page's body is [the markdown editor](#the-markdown-editor), mounted reading, and a click in the prose is the whole gesture. There is no toolbar and no WYSIWYG, because a document is markdown and markdown is text. A `.html` in the directory has a page too and is never writable: olai shows one and never writes one, and it is drawn — and runs — sealed in a sandboxed frame whose origin is nobody's ([format.md](format.md#hypertext)).

**What the page gave up to be one surface**, said plainly, because you will notice it: the editor draws the marks and it does not run the renderer. So on a document's own page a **table** is its pipe text, an **image** is its `![](…)`, a **footnote** is a bracket, a **fenced block** has no syntax colouring inside it, and there are no minted ids for a `#fragment` to land on. Every one of those is still drawn everywhere else the same markdown appears — a note, a day, a chat reply, the document under the node that attaches it — and on this page while the editor is still on its way. Two things were too important to lose and were kept by other means: a **link** is drawn as a link and takes you where it says (the editor puts the app's own address on it), and the **contents** above the document jumps by scrolling the surface to that heading, with the address still naming the section so you can copy it.

The way back for the rest is the editor learning to draw them, not the page keeping a second rendering to swap to.

**Leaving is not what writes it.** A document autosaves: half a second of quiet, or the caret leaving, and what is in the editor is on disk. There is no Save button, no Cancel and no dirty flag — the debounce is idle-keyed, so a pause mid-sentence is not a write and walking away from the keyboard cannot lose the paragraph.

**A draft still belongs to the file it was typed in.** Open another document and what you had typed is flushed to the file you typed it in, on the way out. That is worth saying out loud because the alternative is the quiet kind of wrong — a draft that followed you to the next file could land on it, and where two documents happen to say the same thing (two empty notes, two copies of one file) the conflict guard below would not even notice.

Each write is ONE op at the same gate as everything else: validated, published on its own revision (the other tab showing this document redraws on the frame it lands), audit-trailed, and WAITING in the commit panel like any other write. ⌘Z takes one back, by the same rule as a retyped title: the inverse carries the text it expects to find, so it can only take back what this tab wrote.

**The file can move underneath you, and nothing is clobbered either way.** Edit the same document in vim while you have unsaved words in it and the page says so the moment the disk moves; the next write is refused, in the ops layer's own words, with your text kept exactly where you typed it. The refusal has two doors out and both are yours: take what you need and leave, or press **Overwrite what is there**, which is the same write minus the guard and means exactly what it says. An agent gets the identical story — its `write_document` takes a `was`, and the refusal is the same sentence.

**Two ways to a document that does not exist.** The sidebar's **+ New document** asks for a path — relative, `.md`, judged by the same rules an agent's `create_document` is judged by — and a **bare calendar day** (no node, no note) mints that day's note, filed where your vault already keeps them: the convention is read off the newest existing daily note's own path (`Daily/2026/08/2026-08-12.md` puts September's first note at `Daily/2026/09/2026-09-01.md`), never configured. Either door lands in the new document with the caret already in it, and the sidebar lists the file on the same frame.
