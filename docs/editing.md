...
**The Inbox entry is drawn only when there is an inbox.** A directory that has never captured has none, and minting one is the capture's job, not a door's — so the entry appears the moment your first `⌘K` `+` lands and opens the file that capture went into. It sits **beside Agenda**, and it wears Agenda's own count badge: how many top-level captures the file holds. The chip hides at zero — an empty inbox is a door, not news. It is an ordinary outline behind that door — type into it like any other — so the entry lights up when you are on it and carries the ⚠ when the file will not parse.
