...
- [live-properties.md](live-properties.md) — the one seam both integrations hang off: a property whose value is a name and whose face updates on its own, the DECLARATION in `_olai/Properties.org` that turns one on rather than the key's name, the two shapes a face takes (a block that owns a row, a chip beside the value), what a live face is allowed to be, and what a property whose plugin is switched off is.
- [format.md](format.md) — the file format and its rules: the record shapes, the fields, status, references (`see`, `@<id>` in prose, and what a zoomed node reads backwards), days, the pinned shelf (`Pins.org`, and why a pin is an address rather than a field), which files a served directory is made of (`.org`, `.md`, and the four kinds olai only shows — `.html`, `.csv`, the picture suffixes, `.pdf`), and merge safety.
