...
- [search.md](search.md) — one query language, four doors: what matches, the operators (`is:`, `has:`, `date:`, `-`), the `"quoted phrase"` and the `OR` that compose them, what a result row shows, and the filter that narrows a page in place — every page that draws nodes, and what narrowing means on each.
