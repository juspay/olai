...
- [ci.md](ci.md) — CI on the board: LIVE PROPERTIES, the one seam both of these hang off — a property whose value is a name and whose face updates on its own — and its second tenant, the chip a `worktree` wears while odu is running in that checkout, the run matrix it opens, what a settled run leaves behind, and the two facts on the board that turn it on.
