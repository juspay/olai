...
Phase 2 reads that directory ONCE, at startup. Watching it is phase 3, and the paragraphs below say which parts are built and which are the shape they are being built into.
...
- **Store** (`packages/store`, generic — caller-supplied codec, no olai types; extractable to its own repo later): files load into a validated snapshot, revision-tagged, with errors on a second independent channel so a broken set does not blank what is on screen. Today it loads once. Phase 3 adds staleness detection by probe (file stamps plus re-asked directory listings) with a file watcher that only triggers the probe, so a file arriving via `git pull` appears without a restart, and a broken file keeps the last good snapshot. The API is already that shape — the snapshot is a `SubscriptionRef` whose `changes` is current-value-then-updates — so the watcher lands behind it without a consumer changing a line. Design: [brainstorming/architecture.live-store.md](brainstorming/architecture.live-store.md).
