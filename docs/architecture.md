...
- **The connection is a thing the page reports on.** A page that is live and a page whose server died look identical unless something says otherwise, so a dot in the corner reports the wire in every shape of the app, green only while a server is answering. It is driven by surface-app's `createServerLifecycle`, which probes the surface's own `identity.info` on each socket open and compares the process id — that is what separates a dropped connection (which heals itself) from a replaced server (which cannot). The client echoes the last id it saw as the `?pid` parameter on every re-dial, so a restarted server recognises a tab that predates it and closes it at the handshake; the wire retires for good, and the page says so and offers the one thing that works, a reload. The two halves are one mechanism: without the echo the gate never fires, and without the gate the echo is a parameter nobody reads.
...
| `surface` | `format` | the surface spec: an `outlines` stream, an `errors` cell and an `identity.info` procedure |
