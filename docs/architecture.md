...
|---|---|---|
| `format` | — | the format core: `parseOutline` per file, `validate` per set, and the derivations (status, sibling order, tags, one node's ancestry and what its id zooms to) both the validator and the view read from |
