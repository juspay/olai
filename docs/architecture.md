...
It reads that directory and keeps reading it: edit a file or `git pull`, and the open page follows without a reload. Writing is phase 4; the paragraphs below say which parts are built and which are the shape they are being built into.
