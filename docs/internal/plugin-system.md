...
| **settings** (`Settings`) | a person's runtime configuration, one section per plugin in `_olai/Settings.olai`; schema defaults, then the row's config, then the vault; a secret never reaches a page |
