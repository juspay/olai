...
| **settings** (`Settings`) | a person's runtime configuration, one section per plugin in `_olai/Settings.olai`; schema defaults, then the vault; host facts stay on the row; a field annotated `secret` is refused at register |
