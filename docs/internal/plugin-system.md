...
| ...and `lifecycle.test.ts` beside it | the two claims a bare scope cannot make: that a registration made in a CHILD scope stops with that child while its plugin stays mounted, pending and running alike, and that a gate's two owners join one cut rather than the second finding an empty set |
