...
So on a day and the agenda there is nothing archived for a query to find or for `-is:archived` to take out, and the page a filter runs on is asked about the archive only when it is the trash or one archived node zoomed. The cost of asking is that a page drawing archived rows has the whole archive in front of the matcher, which is the same whole-set scan every other node already gets.
