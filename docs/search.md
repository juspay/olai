...
**`is:blocked` is the one DERIVED value in the grammar**, and it is the derivation the app already draws: the same `blockersOf` that dims a row and writes the `blocked by` line under a node. So it says exactly what the page says, and everything blockedness is made of comes with it rather than being restated here — a target with no mark blocks nothing (a bullet is not work), a `done` one has happened, work in the archive is out of it at both ends, and both spellings of an edge (`after` on the waiting node, `blocks` on the one in the way) are one graph. It crosses files for the same reason: the derivation is of the whole directory, so a node waiting on one in another outline is blocked here as it is on screen. `has:after` is the other question — `order after demo` where `demo` is finished carries the edge and is not waiting on it. `-is:blocked` is what can be started.
