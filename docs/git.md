...
Every commit message starts with `olai`, so `git log --grep '^olai'` is the audit view and `--invert-grep` gives you back your own history. Each commit carries an `X-Olai-Writer` trailer saying which of you — the chat agent, an MCP client, the browser (`web`: the keyboard editor and the row's `•••` menu, which write through the same door), a terminal (`olai surface`), or `auto` for one the quiet window made with nobody pressing anything — wrote it. A commit that swept up files beside the outlines names them in its body, with what happened to each, so the log says what the commit did rather than only what it was about.
