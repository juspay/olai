...
**It is the DIRECTORY's, not this browser's.** The row on the preferences panel is the only one there that is not a claim about you: it sets the policy this *server* runs under for this directory, remembered across restarts, and every tab drawing the panel draws the same answer. Turn it on in one browser and the directory commits itself in every other one — and with none open at all, which is the point. `olai web --commit=auto` is that same row given on the command line ([running.md](running.md#the-git-policy)).
...
- `--commit=auto` IS [Auto-commit](#committing-on-its-own) — the same quiet window, given on the command line instead of pressed in a preferences panel. What is waiting records itself once writes stop arriving for fifteen seconds, and it sweeps the whole repository.
...
Giving `--commit` at all — or `--push`, its neighbour — also **pins** the matching row in every browser's preferences: read-only, in the state that flag comes to, with the flag itself on screen. `--commit=auto` freezes it on, and both `manual` and `off` freeze it Off — `manual` because a write waiting for the Commit button is exactly what Off means, and `off` because a directory olai never commits in has no window to run. The line under the row names the flag either way, so the two Offs are told apart by what set them. That is the server's answer for a directory more than one person is looking at, and it is [running.md](running.md#the-git-policy)'s subject. With neither flag given, which is the default, both rows are live — and they still set the same server's policy, which is simply nobody having ruled it out.

## The audit view

Every commit message starts with `olai`, so `git log --grep '^olai'` is the audit view and `--invert-grep` gives you back your own history. Each commit carries an `X-Olai-Writer` trailer saying which of you — the chat agent, an MCP client, the browser (`web`: the keyboard editor and the row's `•••` menu, which write through the same door), a quick capture, or `auto` for one the quiet window made with nobody pressing anything — wrote it. A commit that swept up files beside the outlines names them in its body, with what happened to each, so the log says what the commit did rather than only what it was about.
