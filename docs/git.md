...
There is no pull and no branch UI, and that is deliberate — this is an audit-trail recorder rather than a git client. What a push DOES, once — exactly once, every time — is take in the upstream first: it fetches, rebases what is unpushed onto the moved branch, and sends, so the upstream's commits land under yours and a push from here is never a non-fast-forward. The take-in runs in a linked worktree beside the served directory, and your tree changes only as the whole move succeeds — nothing is ever lost to a conflict, which stops instead ([When it stops](#when-it-stops)). When git refuses — a conflict, an authentication failure, an integration that could not run, a branch with no upstream — you get git's own words, verbatim, because resolving it is a conversation in a terminal and those words are how it starts. A repository that is mid-rebase says so instead, naming the rebase.
