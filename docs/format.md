...
**Nudges are policy, and they are the writer's, not the format's.** When a mark lands the ops layer may say what the rollup noticed — a branch ticked over tasks that are still open, or the last open task under a parent going done, which is the moment to consider ticking the parent. It travels in the answer to a write that HAPPENED, as data. Nothing here is ever a load invariant: a set that arrives from a git merge has nobody to nudge, and a file that will not load is a worse answer to "these two disagree" than a file that loads and says so.
