...
- It is **shown, never run.** The markup is drawn in an iframe with `sandbox=""` — every restriction on, so no script executes and the frame's origin is nobody's — behind a `default-src 'none'; style-src 'unsafe-inline'` content policy prepended to the markup. A script in somebody's saved page cannot reach this app's origin (no `document.cookie`, no `localStorage`, no reading the page around it), and nothing in the file fetches anything from anywhere. The file's own `<style>` is what makes the preview worth having, so inline CSS is the one thing allowed — and with every fetching directive at `'none'`, CSS has nothing it can reach.
