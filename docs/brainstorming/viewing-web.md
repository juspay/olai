...
Racket parity, and the parity is the *palettes*: fifteen named ones at ship, `chalk` the default, `data-theme` on `<html>`, `localStorage`, no server round-trip (later rows join the same table). What had to be decided here is the vocabulary they are written in and where the CSS comes from.

- **The table is the source, and the CSS is generated from it.** `theme/palettes.ts` is one row per theme; the build appends one block per row to the Tailwind output, and the picker draws one chip per row. Hand-written CSS was the alternative and it is the same eight lines copied once per theme — one place per theme for a new token to be forgotten. The cost is that `styles.css` no longer contains the palette a reader might go looking for; the answer is a test (`theme/css.test.ts`) that holds the eight `@theme` defaults to the default row, since Tailwind can only emit `text-muted` for a `--color-muted` it has *seen*.
...
- **The OS does not vote.** `prefers-color-scheme` is gone from the sheet. It chose the palette before, which meant a page that changed under a reader who had already said what they wanted, and two ways to be dark that could disagree. A page that has picked nothing reads in the default — `chalk`, which promises AA, so the unchosen page is the legible one. `color-scheme` still rides in every block, because the scrollbars and form controls are the browser's to paint.
...
- **Each chip wears the palette it offers.** A column of names cannot say what they look like; three inline styles from the table can. Inline rather than utilities because a class built from data is a class Tailwind never scanned.
