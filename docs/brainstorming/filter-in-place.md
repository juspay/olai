...
**Amended by the durations, and the stance is intact:** a bound is no longer always ten characters, so each bound is compared at its OWN width — the value is cut to the length of the bound it is being tested against. A day bound still reads the day an instant falls on (the old `dayOf`, spelled as the general case), and a duration's bound reads the clock face as well. Still text, still no parse; the arithmetic that mints the moment is `calendar.ts`'s, over integers, once per query.
