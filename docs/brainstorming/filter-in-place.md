...
Every group must hold, and a group holds when ANY of its tokens does. Terms are ANDed as they already are ("every word must appear somewhere in the same node"), and clauses join the same conjunction. A leading `-` negates whichever it is in front of.
