...
Status: DESIGN, awaiting the human's ruling. Written 2026-08-11 for roadmap item `surface-mcp-viewing` (child of `surface-mcp`), design-first by instruction — no product code written. Every claim about `@kolu/surface-mcp` below was verified against **the revision olai actually pins**, `580ab79e8cc7715103af4bddb35d5c9128f897dc` (`npins/sources.json`, 2026-08-10), read with `git show` out of the local kolu clone. Revised the same day to absorb the `snapshot-scale` requirement (roadmap `b794e1d`, on origin/master and not yet in this worktree).
...
3. **The `SurfaceClientCallable` cast** — cosmetic. `buildSurfaceFace`'s `unknown` leaves versus the adapter's callable leaves; upstream's own example snippet carries the cast and calls it a framework follow-up.
