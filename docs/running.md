...
gets the same closed tool list by dialling the running server. There is no
second process and no stdio face: `olai web` owns the store, and `/mcp` is
how an agent that is not the panel's talks to it.
...
`7714` is this repo's convention: `just run` (and `just serve`) bind that
port, spelled once as the `port` variable in the justfile, so the URL in
`.mcp.json` holds. Another directory picks its own port and writes the same
shape. Requests from `127.0.0.1` do not need a bearer token; the chat still
sends the one it was handed, which is accepted and ignored. A request that
did not come from loopback is refused without that token.
...
or marks one. The two write surfaces are a page and an HTTP POST at `/mcp`,
and they are two clients of one server.
