...
```
olai web ~/outlines --plugins=odu                        # odu only — and NO chat panel
...
```nix
  services.olai.plugins = [ "odu" ];              # odu only — and no chat panel
...
**The preferences row says which of five states a plugin is in**, not just on or off. Running is the ordinary one. The other four are all total absence and they differ in *why*: it was not asked for, this build ships it off until you name it (which is what `xyne-spaces` is, and the row names the flag value that turns it on), it was asked for and its **start failed** — in which case the row quotes what the plugin said, verbatim — or it was asked for and is still waiting on something it needs. Only the third is a fault, and it is the one nothing else on screen would tell you about: an integration whose start failed draws nothing at all, exactly like one you turned off on purpose.
