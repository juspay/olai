...
* Hand-written htmx/SSE attributes for a LIVE VIEW (region, link, connection, item) are BANNED in app code — the `live/` forms are the only door, writes are out of scope, and a gap in the vocabulary is proposed to the human, never hacked around: [live/README.md](live/README.md#raw-htmx-attributes-are-banned).
