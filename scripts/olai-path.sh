...
# rather than teaching every recipe a new guard. The packaged binary
# answers the same question in makeWrapper (default.nix's `--prefix PATH`);
# this is the dev loop's spelling of it.
#
...
# set to a directory, it is the prefix instead of the pin; set to the EMPTY
# string, it is the explicit off switch — the ambient PATH answers as-is,
# and a PATH with no odu is then a DRAWN row, by design
# (packages/plugins/olai-plugin-odu/src/probe.ts).
#
