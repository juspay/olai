...
# convention). nixpkgs, the kolu source, and the bun-1.4 nixpkgs PR are
# pinned by npins (npins/sources.json) and imported via fetchTarball, which
# keeps `nix develop` cold eval near a second instead of the several the
# flake input system costs per input. Add a pin, not an input. bun itself
# is overlaid from the `nixpkgs-bun` pin (nix/nixpkgs.nix); drop that pin
# and the overlay when NixOS/nixpkgs#556047 merges (bun-nixpkgs-catchup).
#
