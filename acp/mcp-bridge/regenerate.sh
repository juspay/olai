...
# Rebuild acp/patches/pi-mcp-servers.patch.
#
# Same discipline as the sibling rig (acp/session-list-info/regenerate.sh —
...
#
#     bash acp/mcp-bridge/regenerate.sh

root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
out="$root/acp/patches/pi-mcp-servers.patch"
work="$(mktemp -d)"
trap 'rm -rf "$work"' EXIT

version="$(jq -r '.packages["node_modules/pi-acp"].version' "$root/acp/package-lock.json")"
