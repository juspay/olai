...
This folder is the bridge that answers the hole. The pin's derivation applies `acp/patches/pi-mcp-servers.patch` to the adapter's `dist/index.js`, and the patched spawn hands each session's pi:
...
| `regenerate.sh` | The patches/generation dance (same discipline as `acp/session-list-info/regenerate.sh`). |
...
2. `bash acp/mcp-bridge/regenerate.sh` — the patch re-baseline.
